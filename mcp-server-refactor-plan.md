...
> Last audited against live code: 2026-06-09

## Files

- `lib/mcp-registration.js` — ~7521 lines. Full Pulsar restart + babel cache clear required for registerTool/schema changes. Handler body changes hot-reload on save.
...
- `lib/string-utils.js` — Pure utility functions: `escapeRegex`, `applyReplacement`, `expandBraces`, `globToRegex`, `levenshteinDistance`, `calculateSimilarity`. Hot-reloads on save.
...
### 🔧 MEDIUM — Extend `smartSuggestion` + `logFailure` to remaining edit tools
...
**Related — extend `logFailure` to `insert` and `replace-function-body`:** `logFailure` (NDJSON failure log) is currently only called from the `str_replace` noMatch path. The `insert` anchorNotFound path and `replace-function-body` notFound path both have good diagnostic info already returned to the LLM (line preview / function name list) but nothing is written to `failure-log.ndjson`. Adding `logFailure` calls there (with `diffVsBuffer: null`) would complete the failure capture picture across the main edit tools. Low complexity — two `logFailure(...)` calls with appropriate `reason` and `oldStrPreview` fields.
...
The thing to note here is that we mostly use hints as anchor points not line numbers so this has limited uses except for calls with out hints, but it is still good for context so the llm is aware of this.
...
- The tool framework (Tier 1 above in the TODO list) makes this much easier — once handlers receive `ctx` injection, `mockEditor` is just a mock `ctx`
