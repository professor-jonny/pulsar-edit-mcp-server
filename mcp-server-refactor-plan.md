...
### Matching techniques from Cline / Aider / opencode -- what to implement, investigate, or reject (researched 2026-09-25)

Source: the three reference trees under `test/` (opencode `packages/opencode/src/tool/edit.ts`, Aider `aider/coders/editblock_coder.py`, Cline `apps/vscode/src/core/assistant-message/diff.ts`), read against `lib/match-engine.js`, plus GitHub issue reports for the same features and this project's own `get-failure-log` (200 most recent `str_replace` failures, 2026-07-31 to 2026-09-24) and lifetime `get-edit-stats`. Every item carries its evidence and a status. **Nothing here is built.** Lifetime counters were still doubled when this was written (see the stats double-count fix), so ratios below are valid and absolute counts are roughly 2x.

**What none of the three tools has, and this one does (worth protecting):** scope hints (`inFunction`, `betweenHint`, `afterString`, `occurrence`); refuse-don't-guess on ambiguity and out-of-range `occurrence`; structured failure diagnostics. opencode's only disambiguation is `replaceAll` plus refusing a non-unique match. All three match WHOLE LINES and blocks; none does sub-line character-span matching, so `matchFragment` is this project's own addition (and see the S3 ruling above: part-line matching as a NEW feature for other tools was rejected as too messy).

**IMPLEMENT (worth doing, in this order):**
1. **Fault-log diagnostics -- prerequisite for everything below. STRENGTHENED 2026-09-25 with a live-reproduced second bug, not just the stats-sample gap.** In the 200-entry sample `diffVsBuffer` is empty in 200/200 entries, and `bufferPreview` is the file HEADER (first lines), not the region around the closest match. The two largest failure classes (`noMatch` 62, `whitespace` 52, plus `partialMatch` 24 = 138/200) therefore cannot be diagnosed from the log. Populate `diffVsBuffer` and record the buffer window around the closest match. **Second, sharper finding, live-reproduced against this very file while editing it this session (`mcp-registration.js` L815-865, `str_replace`/`delete`'s shared failure-diagnostic builder -- NOT `match-engine.js`; `calculateSimilarity` itself, `string-utils.js` L77-101, is a correct plain Levenshtein ratio, verified directly, no bug there):** the "Closest area found" / "Similarity: N%" block is only as good as `fuzzyRow`, a word-overlap scorer (L818-823, counts how many of `old_str`'s words appear in each candidate line, best score wins) that picks which buffer row to diagnose against. On a file with repetitive vocabulary and/or very long single lines (this plan doc qualifies on both counts -- words like "wire"/"commit"/"Section" recur constantly, and several lines run 1500+ characters as one paragraph), `fuzzyRow` can lock onto the WRONG row, and every downstream field (`Closest area found` context, the `Similarity` percentage, the `simHint` advice tier) inherits that wrong row silently -- nothing in the output flags that the located row might be wrong. Result: a confidently-specific but WRONG percentage (e.g. "2%", "7%" both observed live this session against known-correct `old_str`/`startContent` values that a direct `grep-file`/`read-file` confirmed were present verbatim) that reads as "old_str may be pointing at the wrong location entirely" when the real cause is the locator, not the caller's input. This is worse than the missing-diffVsBuffer gap above: a confidently-wrong diagnostic actively misleads (cost several extra tool calls chasing a false file-corruption theory before the real cause was found by reading the source directly) whereas an honestly-empty one does not. Fix, in addition to populating `diffVsBuffer`: give `fuzzyRow` a plausibility floor -- only emit the `Closest area found`/`Similarity` block when the winning row's word-overlap score clears some absolute minimum (not just "best of a bad field"), otherwise say "no clearly closest region found" rather than a specific-looking wrong number. Cheap, additive, no behaviour change to the actual match/commit path (this code only runs after a match has already failed, so it cannot have caused a wrong edit or data loss -- diagnostic-only). Also record the tool's hit count next to failures so the log has a denominator.

2. **Disproportionate-match guard for fuzzy paths** (opencode `isDisproportionateMatch`, edit.ts L731-737): refuse a fuzzy/whitespace-tolerant match whose span is much larger than `old_str` (matched lines >= max(old lines + 3, old lines * 2), or for multi-line needles matched length > max(old length + 500, old length * 4)). This project's fuzzy tiers have no equivalent. It is a safety rule with no downside and no failure-rate claim attached. Add to the engine's fuzzy paths and unit-test with a negative control.
3. **Audit what `fuzzyWhitespace` does to indentation on commit.** Lifetime `str_replace`: 676 `fuzzyWhitespaceCommits` against 2032 hits, `fuzzyTriggerReasons.needsWhitespace` 410. CAVEAT: the Edit Strategy Notes make `fuzzyWhitespace:true` the standing default for `mcp-registration.js` (mixed indentation), so this count is inflated by one heavily-edited file and is NOT evidence about matching in general. What is unknown is whether a `fuzzyWhitespace` commit re-indents the replacement or only trims for matching (opencode's `IndentationFlexibleReplacer` and Aider's `replace_part_with_missing_leading_whitespace` both strip a uniform indent, match, then re-indent the replacement). Read `recover.js`/`applyEdit` first; only build if the audit finds mangled indentation. Blocked on item 1 for real-world evidence.

**INVESTIGATE FIRST (do not build yet):**

4. **CRLF / BOM preservation in `applyEdit` and `applyEditSpan`.** No CRLF or BOM handling was found in `match-engine.js` by grep. Evidence it matters here: `lib/mcp-registration.js` begins with a U+FEFF BOM (`'use babel'`), `.xml` files appear among failures, and earlier notes record mojibake in this file. Evidence against: only 1/200 failures mentions CR. opencode's own edit.ts normalises `\r\n`, matches, and converts the replacement back, but its GitHub record shows this is an unreliable model to copy (issues #6348 edit tool messes with whitespace/line endings on Windows; #50740 edit tool inflated CRLF files with blank lines on repeated edits; #31224 write tool ignores line endings; several third-party plugins exist only to patch it). Write the capability carefully here rather than copying: first add a regression test (CRLF file, BOM file, mixed-ending file) against the current engine to see what actually happens, then decide.
5. **Why `delete` succeeds only ~13% of the time and `inFunction` only ~45%.** Lifetime: `delete` 32 hits / 208 fails (`anchorNotFound` 50, `startNotFound` 48, `anchorAmbiguous` 34, `noMatch` 22, `whitespace` 20, `endNotFound` 16); `str_replace` hint success `inFunction` 30 ok / 36 fail, `betweenHint` 6 ok / 4 fail (small sample), against `afterString` 294 ok / 26 fail (92%). Any new range-replace mode built on `startContent`/`endContent`/`betweenHint` inherits this anchor resolution, so understand the failures before reusing it. Overlaps the open `inFunction` investigation in the session notes.

**REJECTED (with reasons -- do not relitigate without new evidence):**

- **Block-anchor similarity matching** (opencode `BlockAnchorReplacer` and `ContextAwareReplacer`, Cline's `blockAnchorFallbackMatch`): match a block by its first and last line and accept the middle by similarity (0.65 Levenshtein / 50% of lines). Known-harmful upstream: opencode #2433 (inserts doubled closing braces, syntax errors) led to it being disabled; #1261 (ambiguity check only protects the exact-match path, so fuzzy replacers can silently pick the wrong occurrence and corrupt a file); an unrelated tool (pi-edit-guard #2) shows the same failure, a token-overlap match silently editing the wrong function and still reporting success. It also contradicts this project's refuse-don't-guess principle. `betweenHint`/`startContent`+`endContent` do the same job deterministically.
- **Escape-normalised matching** (opencode `EscapeNormalizedReplacer`): fixes over-escaped `\n`/`\"` in `old_str`. Only 3/200 failures contain literal backslash-escape text and 2 of those are `fspec.hh`, whose source really contains a `\t` escape in a comment. `fuzzyContentCommits` is 8 lifetime, `regexCommits` 30, `autoStripCommentCommits` 0: the exotic tiers are barely used.
- **Aider's `...` elision** (`try_dotdotdots`): a chat-format feature for models that abbreviate code inside a SEARCH block; not applicable to a structured tool call.
- **Aider's fuzzy edit-distance fallback:** dead code upstream (a bare `return` at editblock_coder.py L183 precedes it), so there is no working reference behaviour to copy.
- **Aider's `did you mean these lines?`** is already covered by (and weaker than) `lib/fail-diagnostics.js` closest-area output.

**OPEN DECISIONS FOR PJ (recorded, not resolved -- raised 2026-09-25 while planning the port):**

- **A. Parallel tool vs grow in place.** PJ asked whether to build a NEW tool (working name `replace`, able to match blocks with `betweenHint` like `read`) and port parts of `str_replace` across so the two can be compared. The plan's standing direction is the opposite: `str_replace` is the reference implementation and only its internals move (see S4e and "What NOT to do": no rename, no alias, no cutover). A separate `replace` tool does not violate those (it is a new tool, not a rename) but adds a third live replace-family tool next to `str_replace` and `replace-block`, and the T4 "kill `replace-block`" question is still unresolved. Trade-off: a parallel tool gives a clean side-by-side comparison with zero risk to `str_replace`'s traffic (2032 lifetime hits, doubled); growing in place follows the staged S4 plan. If built, it should live in its own file (`lib/replace-tool.js`) with its own stats namespace so counters are not mixed with `str_replace`.
- **B. Is sub-line (fragment) matching acceptable in a new tool?** The S3 ruling (PJ, 2026-09-20) rejected part-line search/replace as a NEW feature for other tools. None of Cline, Aider or opencode has it. If that ruling stands, a new `replace` tool would offer whole-line and range (anchor pair) modes only and fragment matching stays inside `str_replace`.
- ~~C. Section 3 commit half~~ -- RESOLVED 2026-09-25. Checked both sources directly: `lib/_s4e_section3_fragment.js` header (L1-19) is dated 2026-09-24, the SAME day as the S4e status bullet below, and is the primary decision record, not a stale one -- it states PJ's reasoning (existing block already does B39/B40; swapping replaces proven code with never-run code; fuzzy/regex/multi-line branches need the old block regardless, so a second commit path exists either way; this file has silently lost work and shipped real bugs at every wiring step so far) and explicitly frames the resulting two-commit-path split as INTENTIONAL, not unfinished work to close later. The S4e status bullet (L177 as of this writing) still poses it as an open either/or -- that bullet is the stale one and should be corrected, not this header. COMMIT HALF STAYS UNWIRED. `commitFragmentEdit`/`applyEditSpan` are not to be wired into str_replace's live commit path. Any future multi-line character-span matching work (see item 5/range-replace above) still needs its own commit logic and cannot assume this gets wired first.

Suggested sequence if A and B are answered "grow in place": items 1, 2 (additive, no behaviour change) first; then 4 as a test-only investigation; then 3 and 5 once item 1 has produced real diffs. A new range-replace capability (`betweenHint`/`startContent`+`endContent` with no `old_str`) is a real gap versus `read`'s pair mode, but belongs after item 5.
