 ...
 *       hints.lineHint    — prefer symbol whose startRow is closest
 ...
 *   lineHint    {number}  — 0-based row; prefer closest match
...
function findFunction(symbols, name, hints = {}) {
  const { occurrence = 1, lineHint, afterRow, betweenRows } = hints;
  ...
  // lineHint: pick closest
  if (lineHint != null) {
    let best = candidates[0];
    let bestDist = Math.abs(best.startRow - lineHint);
    for (const c of candidates) {
      const dist = Math.abs(c.startRow - lineHint);
