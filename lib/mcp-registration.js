  ...
  if (g('edit')) {
  {
    ...
    server.registerTool(
      ...
      async ({ old_str, new_str, functionHint, lineHint, afterHint, betweenHint, occurrence = 1, fuzzyWhitespace = false, dryRun = false }) => {
        ...
        const _isCodeFile2 = /\.(js|ts|jsx|tsx|c|cpp|h|cs|py|java|go|rs)$/i.test(editor.getPath() || "");
        const _ambig = ambiguityCheck({
          ...
          toolName: curTool, isCodeFile: _isCodeFile2,
        });
        if (_ambig) { bump('str_replace', 'fails.partialMatch'); strReplFailures.count++; return _ambig; }
        ...
        const _isCodeFile = /\.(js|ts|jsx|tsx|c|cpp|h|cs|py|java|go|rs)$/i.test(editor.getPath() || "");
        const _nudge = successNudge({
          toolName: curTool,
          noHintsUsed: !functionHint && !afterHint && !betweenHint && !lineHint && occurrence <= 1,
          fileLines: allLines.length,
          oldStr: old_str,
          isCodeFile: _isCodeFile,
        });

        return {
          content: [{ type: "text", text: `✅ Replaced match at line ${matchLine + 1}${scopeLabel}${occurrence > 1 ? ` (occurrence ${occurrence})` : ""}${fuzzyWhitespace ? " [fuzzyWhitespace]" : ""}.${_nudge}` }],
          matched: true,
          dryRun: false,
...

  {
    ...
    server.registerTool(
      curTool,
      {
        ...
        description: [
          "Delete all lines from startLine to endLine (inclusive). CORRUPTION RISK: line numbers shift after every delete — if you call this twice using stale line numbers the second delete will hit the wrong lines and silently corrupt the file. You MUST call get-document or read-lines after each delete to get updated line numbers before any further line-based edits. Returns newLineCount so you can verify the shift.",
          "HINT MODES (content-stable, immune to line drift): functionHint — delete the entire named function body. betweenHint:{start,end} — resolve startLine+endLine from two anchor strings. afterHint — set startLine to the line after an anchor string (combine with endLine). lineHint — alias for startLine.",
          "occurrence:N targets the Nth occurrence when using anchor hints. fuzzyWhitespace ignored (line-based delete). Set dryRun:true to preview without writing.",
        ].join(" "),
        inputSchema: {
