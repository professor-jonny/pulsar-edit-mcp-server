...
export function mcpRegistration(server) {
    ...
    server.registerTool(
      curTool,
      {
        ...
        description: [
          ...
          "Returns the line number where the replacement was made so you can verify the right occurrence was changed."
        ].join(" "),
        inputSchema: {
          query:         z.string(),
  ...
  // ---------------------------------------------------------------------------
  // run-command: cross-platform shell execution (PowerShell on Windows, /bin/sh on Linux/Mac)
  // ---------------------------------------------------------------------------
    ...
    server.registerTool(curTool,
      {
        title: "Grep File",
        description: [
          "Search for a pattern in a specific file (or the active editor if no filePath given) and return matching lines with their 1-based line numbers.",
    ...
    server.registerTool(curTool,
      {
        ...
        description: [
          ...
          "Omit filePath to read from the active editor.",
    ...
    server.registerTool(curTool,
      {
        ...
        description: [
          ...
          "Supports regex and case-insensitive matching.",
          "Returns matchCount so you can verify how many occurrences were replaced."
        ].join(" "),
        inputSchema: {
          query:         z.string(),
          replacement:   z.string(),
  ...
  // ---------------------------------------------------------------------------
  // NEW: insert-at-line (clean alias for insert-text-at-line)
  // ---------------------------------------------------------------------------
    ...
    server.registerTool(curTool,
      {
        ...
        description: [
          ...
          "Alias for insert-text-at-line with the same semantics.",
