...
function mcpRegistration(server, linterRegistry = null, getMessages = null, groups = {}, chatPanel = null) {
  ...
  registerMcpTool({
    ...
    handler: async ({ old_str, new_str, functionHint, lineNumberHint, lineContentHint, afterHint, betweenHint, inFunction, inSymbol, afterFunction, beforeFunction, afterSymbol, beforeSymbol, afterString, beforeString, afterLine, beforeLine, occurrence = 1, fuzzyWhitespace = false, fuzzyContent = false, regex = false, dryRun = false, lint = false }, ctx) => {
      ...
      const isCodeFile = isCodeFilePath(editor.getPath());
        ...
        if (functionHint) {
          ...
          if (!fn) {
            ...
            return {
              content: [{ type: "text", text: [
                ...
                `Functions detected:\n${knownFns.join("\n") || "  (none)"}`,
                smartSuggestion({ toolName: curTool, counter: ctx.consec, noHintsUsed: false, fileLines: allLines.length, oldStr: old_str, isCodeFile: isCodeFilePath(editor.getPath()) })
              ].filter(Boolean).join("\n") }],
              matched: false
            };
          }
          ...
          if (err) {
            ...
            return { content: [{ type: "text", text: [err, smartSuggestion({ toolName: curTool, counter: ctx.consec, noHintsUsed: false, fileLines: allLines.length, oldStr: old_str, isCodeFile })].filter(Boolean).join("\n") }], matched: false };
          }
        ...
        if (matchIndex === -1 && !regex) {
          ...
          if (commentMatch) {
            ...
            // BUG-B guard: refuse autoStripComment rescue when the stripped needle
            // is too short to be a reliable anchor. A very short strippedLast
            // (e.g. a single token or empty) will fuzzy-match on the wrong line
            // inside the hint window, silently committing to the wrong location.
            // Threshold: strippedLast must have =8 non-whitespace chars, and the
            // full stripped needle =10 non-whitespace chars.
            const STRIP_MIN_LAST  = 8;
            const STRIP_MIN_TOTAL = 10;
            const strippedLastNWS  = strippedLast.replace(/\s/g, '').length;
            const strippedTotalNWS = stripped.replace(/\s/g, '').length;
            if (strippedLastNWS >= STRIP_MIN_LAST && strippedTotalNWS >= STRIP_MIN_TOTAL) {
            const searchText  = text.substring(searchStart, searchEnd);
            const searchLines = searchText.split('\n');
        ...
        const _hasScope = functionHint || inFunction || inSymbol ||
          afterHint || betweenHint || lineNumberHint || lineContentHint ||
        ...
        const _nudge = successNudge({
          ...
          lineNumberHintOnly: (lineNumberHint || lineContentHint) && !functionHint && !inFunction && !inSymbol && !afterHint && !betweenHint && !afterFunction && !beforeFunction && !afterSymbol && !beforeSymbol && !afterString && !beforeString,
  ...
  if (g('search')) registerMcpTool({
    ...
    handler: async ({ query, regex = false, caseSensitive = false, maxMatches = 200, contextLines = 0, occurrence = 0 }, ctx) => {
      ...
      catch (e) { ctx.fail(`Invalid regex: ${e.message}`); }
      ...
      if (occurrence > 0 && matches.length === 0) {
        ...
        ctx.fail(`occurrence ${occurrence} not found \u2014 only ${globalIndex} match(es) in file.`);
      }
  ...
  if (g('edit')) registerMcpTool({
    ...
    handler: async ({ insert_line, new_str, endOfFile = false, afterFunction, beforeFunction, afterSymbol, beforeSymbol, afterString, beforeString, afterContent, beforeContent, functionHint, functionEnd, occurrence = 1, dryRun = false, lint = false }, ctx) => {
      ...
      if (functionEnd !== undefined) {
        ...
        const _styleSuffixFE = applyStyleCheck(new_str, editor.getPath());
        return {
      ...
      if (afterContent !== undefined || beforeContent !== undefined) {
        ...
        if (!hit) {
          ...
          bump('insert', 'fails.anchorNotFound');
          logFailure({
            tool: 'insert', reason: 'anchorNotFound', filePath: editor.getPath(),
            lineNumberHint: null,
          ...
          return { content: [{ type: "text", text: [
            ...
            `\n\u26A0\uFE0F ${fn ? `Start of "${functionHint}"` : "Top of file"} \u2014 pick a single unique anchor line from here:\n${ctx2}`,
            `\n\uD83D\uDCA1 Tip: use a single unique line as anchor. Use functionHint to scope to a function body.`,
            smartSuggestion({ toolName: "insert", counter: ctx.consec, noHintsUsed: !functionHint && !afterContent && !beforeContent, fileLines: lineCount, oldStr: null, isCodeFile })
      ...
      if (dryRun) {
        ...
        const before   = allLines.slice(ctxStart, insert_line - 1).map((l, i) => `${String(ctxStart + i + 1).padStart(4)}   ${l}`);
        const inserted = new_str.split("\n").map(l => `${" ".repeat(4)} + ${l}`);
        const after    = allLines.slice(insert_line - 1, ctxEnd + 1).map((l, i) => `${String(insert_line + i).padStart(4)}   ${l}`);
        return { content: [{ type: "text", text: [
  ...
  if (g('edit')) registerMcpTool({
    ...
    handler: async ({ startLine, endLine, dryRun = false, functionHint, afterHint, lineNumberHint, betweenHint, occurrence = 1, fuzzyWhitespace, lint = false }, ctx) => {
            const { editor, buffer, allLines } = ctx;
      ...
      if (functionHint) {
        ...
        const syms  = getSymbols(editor, text2, editor.getPath());
        ...
        const text2 = buffer.getText();
        const syms  = getSymbols(editor, text2, editor.getPath());
        const r = resolveAnchor(afterHint, syms, text2);
      ...
      if (startLine < 1 || endLine < 1) {
        ...
        return { content: [{ type: "text", text: [
          ...
          smartSuggestion({ toolName: curTool, counter: ctx.consec, noHintsUsed: !functionHint && !afterHint && !betweenHint && !lineNumberHint, fileLines: lineCount, oldStr: null, isCodeFile })
  ...
  if (g('edit')) registerMcpTool({
    ...
    handler: async ({ startContent, endContent, sectionHint, preprocBlock, preprocSide, inclusive = true, functionHint, afterHint, betweenHint, lineNumberHint, occurrence = 1, dryRun = false, lint = false }, ctx) => {
            const { editor, buffer, allLines } = ctx;
      ...
      if (sectionHint !== undefined || preprocBlock !== undefined) {
        ...
        if (!resolved) {
          ...
          return { content: [{ type: "text", text: [
            ...
            smartSuggestion({ toolName: curTool, counter: ctx.consec, noHintsUsed: !sectionHint && !preprocBlock && !functionHint, fileLines: lineCount, oldStr: null, isCodeFile })
          ].filter(Boolean).join("\n") }], deleted: false };
        }
      ...
      if (!startHit) {
        ...
        return { content: [{ type: "text", text: [
          ...
          smartSuggestion({ toolName: curTool, counter: ctx.consec, noHintsUsed: !functionHint && !sectionHint && !preprocBlock, fileLines: lineCount, oldStr: null, isCodeFile })
        ].filter(Boolean).join("\n") }], deleted: false, consecFailures: ctx.consec.count };
      }
      const startRow     = startHit.row;
  ...
  if (g('edit')) registerMcpTool({
    ...
    handler: async ({ anchor, newBody, occurrence = 1, dryRun = false, functionHint, afterHint, lineNumberHint, betweenHint, fuzzyWhitespace, lint = false }, ctx) => {
            const { editor, buffer, allLines } = ctx;
      ...
      if (anchorRow === -1) {
        ...
        return { content: [{ type: 'text', text: [
          ...
          smartSuggestion({ toolName: curTool, counter: ctx.consec, noHintsUsed: !functionHint && !afterHint && !betweenHint && !lineNumberHint, fileLines: allLines.length, oldStr: anchor, isCodeFile })
        ].filter(Boolean).join('\n') }], found: false };
      }
      ...
      async ({ command, cwd, timeout = 30000, confirm = false, showOutput = true }) => {
        ...
        if (!confirm && DESTRUCTIVE_RE.test(command)) {
          ...
          const userChoice = await new Promise((resolveChoice) => {
            ...
            wrapper.innerHTML = `<span>?? Destructive command: <code>${command.replace(/</g,'&lt;')}</code> — Run anyway?</span>`;
      ...
      async ({ query, replacement, regex = false, caseSensitive = false, glob = "",
        ...
        if (committing) {
          ...
          if (rafPending.key !== pendingKey || rafPending.files.length === 0) {
            return { content: [{ type: "text", text:
              "?? No matching preview found to confirm. Run the tool without confirm:true first to generate a match listing, then confirm."
            }] };
          }
...
// ── replace-function-body (migrated to Tool Framework) ───────────────────
  if (g('edit')) registerMcpTool({
    ...
    handler: async ({ name, newBody, dryRun = false, occurrence = 1, functionHint, afterHint, lineNumberHint, betweenHint, fuzzyWhitespace, lint = false }, ctx) => {
      const { editor, buffer, allLines } = ctx;
      ...
      if (!found) {
        ...
        logFailure({
          tool: 'replace_function_body', reason: 'notFound', filePath: editor.getPath(),
          lineNumberHint: lineNumberHint || null,
  ...
  registerMcpTool({
    ...
    async handler({ startLine, endLine, filePath, centerLine, radius, functionHint, afterHint, betweenHint, lineNumberHint }) {
      let text;
      let resolvedPath = filePath;
      if (filePath) {
        try { text = await readFileOrBuffer(filePath); }
...
// ── sed (migrated to Tool Framework) ─────────────────────────────────────
  ...
  if (g('edit')) registerMcpTool({
    ...
    handler: async ({ expression, functionHint, dryRun = false, lint = false }, ctx) => {
            const { editor, buffer, allLines } = ctx;
      const curTool  = 'sed';
      const lineCount = allLines.length;
      ...
      if (matchCount === 0) {
        ...
        parts.push(smartSuggestion({ toolName: "sed", counter: ctx.consec, noHintsUsed: !functionHint, fileLines: lineCount, oldStr: expression, isCodeFile: isCodeFilePath(editor.getPath()) }));
        ...
        if (result === false) {
          ...
          if (hunks.length === 0) {
            return { content: [{ type: "text", text: [
              ...
              smartSuggestion({ toolName: "apply-patch", counter: ctx.consec, noHintsUsed: true, fileLines: bufLines.length, oldStr: null, isCodeFile: isCodeFilePath(editor.getPath()) })
          ...
          parts.push(smartSuggestion({ toolName: "apply-patch", counter: ctx.consec, noHintsUsed: true, fileLines: bufLines.length, oldStr: null, isCodeFile: isCodeFilePath(editor.getPath()) }));
  ...
  registerMcpTool({
    ...
    handler: async ({ symbol, glob = '**/*.{c,cpp,h,hpp}', definitionsOnly = false, maxMatches = 200, contextLines = 0, occurrence = 0 }, ctx) => {
      const roots = atom.project.getPaths();
      if (!roots.length) ctx.fail('No project root open.');
      ...
      if (occurrence > 0 && matches.length === 0) {
        ...
        ctx.fail(`occurrence ${occurrence} not found \u2014 only ${globalIndex} match(es) in project.`);
      }
      if (matches.length === 0) {
        logFailure('search_symbol', 'noMatch', null, null, null, symbol);
  ...
  registerMcpTool({
    ...
    async handler({ action, note, project, tail }) {
      dbg("session-notes", "ARGS", { action, project, tail, noteLen: note?.length });
      ...
      if (action === "write") {
        ...
        dbg("session-notes", "note written", { project, timestamp: entry.timestamp });
        return {
