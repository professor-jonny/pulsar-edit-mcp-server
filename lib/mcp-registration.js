...
const { walkDir, resolveStructuralAnchor, findFunctionInBuffer, readTextFromFile, retargetEditor, resolveProjectPath } = require('./buffer-helpers'); // T3 (2026-09-17): findAnchor retired — last live caller (delete_block) ported onto resolveScope()/matchContent()
...
function mcpRegistration(server, linterRegistry = null, getMessages = null, groups = {}, chatPanel = null, getCommandTerminalPanel = null) {
      ...
      const _hasScope = !!(inFunction || betweenHint ||
        ...
        // afterString / beforeString — window relative to a line containing text
          ...
          if (r.notFound) {
            ...
            const _mlHint = r.multiLine ? `  Tip: ${hintName} matched as a multi-line block (${hintVal.split('\n').length} lines) — prefer a SINGLE unique line for reliability.` : '';
            return { content: [{ type: 'text', text: [`? ${hintName}: "${hintVal.substring(0, 60)}" not found in file.${_mlHint}`, smartSuggestion({ toolName: curTool, counter: ctx.consec, noHintsUsed: false, fileLines: allLines.length, oldStr: old_str, isCodeFile })].filter(Boolean).join('\n') }], matched: false };
          }
          if (r.ambiguous) {
          ...
          if (isMultiLine) {
            ...
            for (let i = fnStartRow; i <= fnEndRow && i < allLines.length; i++) {
              if (allLines[i].includes(effectiveOldStr)) {
                occurrencesFound++;
                allOccurrenceLines.push(i + 1); // 1-based
                if (occurrencesFound === occurrence) {
                  matchLine  = i;
            ...
            consecFailures: ctx.consec.count
                ...
                if (matchIndex === -1 && !regex) {
                  ...
                  if (anyTransform) {
                    ...
                    if (fuzzyStart !== -1 && fuzzyStart2 !== -1) {
                      // B41 guard: two similar-enough lines matched the fuzzy scan within
                      // the same search window. Taking fuzzyStart (the first) silently would
                      // risk editing the wrong one with no warning. Refuse instead.
                      ctx.consec.count++;
                      bump('str_replace', 'fails.ambiguous'); bump('str_replace', 'faultBuckets.hintFaults');
                      return {
                        content: [{ type: "text", text: [
        ...
        if (matchIndex === -1 && !regex && old_str.includes('\n')) {
          ...
          // B41 guard: pmLeader is looked up against the ENTIRE buffer with no scope
          // and no ambiguity check (unlike the exact-match B38 guard above, which only
          // covers the plain-string branches). If the same leading-lines text appears
          // more than once anywhere in the file, silently taking the first hit risks
          // editing the wrong occurrence with no warning. Check for a second occurrence
          // before committing to this rescue.
          const pmLeaderIdx2 = pmLeaderIdx !== -1 ? text.indexOf(pmLeader, pmLeaderIdx + 1) : -1;
          if (pmLeaderIdx !== -1 && pmLeaderIdx2 !== -1) {
            ctx.consec.count++;
            bump('str_replace', 'fails.ambiguous'); bump('str_replace', 'faultBuckets.hintFaults');
            const _line1 = text.substring(0, pmLeaderIdx).split('\n').length;
            const _line2 = text.substring(0, pmLeaderIdx2).split('\n').length;
            return {
              content: [{ type: "text", text: [
                `\u26a0\ufe0f str_replace: the partial-match rescue found similar text in more than one place (at least lines ${_line1} and ${_line2}) -- editing the first would be ambiguous, so nothing was changed.`,
  ...
  if (g('edit')) registerMcpTool({
    ...
    group:    'edit',
    category: 'edit',
    features: { dryRun: true, lint: true, styleCheck: true, consecutiveFailureCounter: true, successNudge: true, structCheck: true },
    title:    'Insert',
    description: [
    ...
    handler: async ({ afterLine, beforeLine, onLine, expectedContent, new_str, endOfFile = false, startOfFile = false, afterFunction, beforeFunction, afterSymbol, beforeSymbol, afterString, beforeString, betweenHint, afterContent, beforeContent, inFunction, sectionHint, preprocBlock, preprocSide, hintRadius, occurrence, fuzzyWhitespace = false, dryRun = false, lint = false }, ctx) => {
      ...
      if (afterFunction !== undefined || beforeFunction !== undefined ||
        ...
        const dir = hintName === 'betweenHint' ? 'after' : (hintName.startsWith('after') ? 'after' : 'before');
        ...
        const scope = resolveScope({
          ...
          params: { afterFunction, beforeFunction, afterSymbol, beforeSymbol, afterString, beforeString, betweenHint, occurrence, hintRadius: _radius, fuzzyWhitespace },
        });
        ...
        const insertRow  = dir === 'after' ? scope.anchorRow + 1 : scope.anchorRow;
        const scopeLabel = `${scope.scopeLabel || ` ${hintName} anchor`} (anchor line ${scope.anchorRow + 1})`;

        if (dryRun) {
          return buildInsertPreview({ allLines, lineCount, insertRow, new_str, suffix: scopeLabel });
        }
      ...
      // -- sectionHint / preprocBlock structural anchors ---------------------
      if (sectionHint !== undefined || preprocBlock !== undefined) {
        ...
        bump('insert', `hintsUsed.${hintName}`);
        if (occurrence > 1) bump('insert', 'hintsUsed.occurrence');
        const resolved = resolveStructuralAnchor(buffer, { sectionHint, preprocBlock, preprocSide, occurrence });
        if (!resolved) {
          bump('insert', 'fails.anchorNotFound');
          logHintFailure({ tool: curTool, hintName, hintValue, reason: 'notFound', filePath: ctx.filePath, oldStr: null });
          return { content: [{ type: "text", text: `\u274C ${hintName}: "${hintValue}" not found. Use get-structural-anchors to list available anchors.` }], inserted: false };
        }
        if (resolved.ambiguous) {
          bump('insert', 'fails.anchorAmbiguous');
          logHintFailure({ tool: curTool, hintName, hintValue, reason: 'ambiguous', filePath: ctx.filePath, oldStr: null });
          const lines = resolved.matches.map(m => m.startRow + 1).join(', ');
          return { content: [{ type: "text", text: `\u274C ${hintName}: "${hintValue}" matches ${resolved.matches.length} places (lines ${lines}). Narrow the hint or add occurrence:N to pick one.` }], inserted: false };
        }
        const insertRow  = resolved.endRow + 1;
        const scopeLabel = ` after ${hintName} "${hintValue}"`;
        if (dryRun) {
          return buildInsertPreview({
            allLines, lineCount, insertRow, new_str,
            suffix: `${scopeLabel} (after line ${resolved.endRow + 1})`,
            beforeOffset: 1,
          });
        }
        const textWithNewline = new_str.endsWith("\n") ? new_str : new_str + "\n";
        const _preSnapFE = preEditSnapshot(editor);
        buffer.insert([insertRow, 0], textWithNewline);
        const _structSuffixFE2 = postEditDelta(_preSnapFE, editor).struct;
        ctx.consec.count = 0; bump('insert', 'hits');
        bump('insert', `hintsSucceeded.${hintName}`);
        if (occurrence > 1) bump('insert', 'hintsSucceeded.occurrence');
        const newLineCount = buffer.getLineCount();
        const _lintSuffixFE  = await maybeLintSuffix(lint, editor, insertRow, insertRow + new_str.split("\n").length - 1);
        const _styleSuffixFE = applyStyleCheck(new_str, ctx.filePath, insertRow);
        const _nudgeFE = successNudge({
          toolName: curTool,
          noHintsUsed: false,
          afterLineOnly: false,
          matchedLineContent: allLines[resolved.endRow] || '',
          fileLines: allLines.length,
          oldStr: null,
          isCodeFile,
        });
        return {
          ...(await buildEditResponse({ tool: curTool, line: insertRow + 1, linesChanged: new_str.split('\n').length, scopeLabel, buffer }, { nudge: _nudgeFE, lint: _lintSuffixFE, style: _styleSuffixFE, struct: _structSuffixFE2 })),
          dryRun: false, newLineCount
        };
      }
      ...
      if (afterContent !== undefined || beforeContent !== undefined) {
        ...
        const match = matchContentEngine(anchor, { allLines, text: ctx.text }, scope, { fuzzyWhitespace, occurrence });
  ...
  if (g('edit')) registerMcpTool({
    ...
    description: [
      ...
      "(4) Know unique text at the start and end of the block? \u2192 startContent:'...' and endContent:'...' \u2014 deletes from startContent line to endContent line inclusive. inclusive:false excludes the anchor lines themselves.",
    ...
    handler: async ({ startContent, endContent, sectionHint, preprocBlock, preprocSide, inclusive = true, matchString, startLine, endLine, expectedContent, inFunction, afterFunction, beforeFunction, afterSymbol, beforeSymbol, afterString, beforeString, afterLine, beforeLine, betweenHint, hintRadius, occurrence, fuzzyWhitespace, fuzzyContent, regex, dryRun = false, lint = false }, ctx) => {
      ...
      // -- Mode 1/2: structural anchor pre-pass (sectionHint / preprocBlock), ported from delete-block --
      if (sectionHint !== undefined || preprocBlock !== undefined) {
        ...
        if (resolved.ambiguous) {
          ctx.consec.count++;
          bump('delete', 'fails.anchorAmbiguous');
          const lines = resolved.matches.map(m => m.startRow + 1).join(', ');
          return { content: [{ type: "text", text: `\u274C ${kind} matches ${resolved.matches.length} places (lines ${lines}). Narrow the hint or add occurrence:N to pick one.` }], deleted: false };
        }
      ...
      if (inFunction && !startContent && !endContent && !matchString) {
        ...
        const delStart = fn.startRow;
        const delEnd   = fn.endRow;
        const deletedCount = delEnd - delStart + 1;
        const r  = 3;
        const cs = Math.max(0, delStart - r);
        const ce = Math.min(lineCount - 1, delEnd + r);
      ...
      if (startContent && endContent) {
        ...
        const _startMatch = matchContentEngine(startContent, { allLines, text: ctx.text }, _startScope, {});
        ...
        if (!startHit) {
          ctx.consec.count++;
          bump('delete', 'fails.startNotFound');
        ...
        const _endMatch = matchContentEngine(endContent, { allLines, text: ctx.text }, _endScope, {});
      ...
      if (matchString !== undefined) {
        ...
        const _match = matchContentEngine(matchString, { allLines, text: ctx.text }, _scope, { fuzzyWhitespace, fuzzyContent, regex });
        if (!_match.matched) {
          ctx.consec.count++;
          bump('delete', 'fails.matchStringNotFound');
      ...
      let _resolvedVia = (startLine != null || endLine != null) ? 'positional' : null;
      if (betweenHint) {
        bump('delete', 'hintsUsed.betweenHint');
        ...
        if (errStart) { bump('delete', 'fails.outOfRange'); return { content: [{ type: "text", text: errStart }], deleted: false }; }
        const rEnd = resolveAnchor(betweenHint.end, syms, text2);
        const errEnd = anchorError('betweenHint.end', betweenHint.end, rEnd);
        if (errEnd) { bump('delete', 'fails.outOfRange'); return { content: [{ type: "text", text: errEnd }], deleted: false }; }
        startLine = rStart.row + 1;
        endLine   = rEnd.row   + 1;
        _resolvedVia = 'betweenHint';
      } else if (afterString) {
        bump('delete', 'hintsUsed.afterString');
        const r = resolveStringPosition(allLines, afterString, 'after', _radius);
        if (r.notFound) { bump('delete', 'fails.outOfRange'); const _mlHint = r.multiLine ? `  Tip: afterString matched as a multi-line block (${afterString.split('\n').length} lines) — prefer a SINGLE unique line for reliability.` : ''; return { content: [{ type: "text", text: `\u274C afterString: "${afterString}" not found in active editor.${_mlHint}` }], deleted: false }; }
        if (r.ambiguous) { bump('delete', 'fails.outOfRange'); return { content: [{ type: "text", text: `\u274C afterString: "${afterString}" is ambiguous (found on lines ${r.rows.map(n => n+1).join(', ')}).` }], deleted: false }; }
        startLine = r.anchorRow + 2;
        // afterString only pins startLine — endLine still comes from the raw
        // param, so this mode is half drift-immune / half positional.
        _resolvedVia = 'afterString+endLine';
      }
      if (occurrence > 1) bump('delete', 'hintsUsed.occurrence');
      if (fuzzyWhitespace) bump('delete', 'hintsUsed.fuzzyWhitespace');
      ...
      // T3: optional content verification for the raw-line / half-positional
      // paths. Neither 'positional' (raw startLine+endLine) nor
      // 'afterString+endLine' (endLine still comes from the raw param even
      // though startLine is content-anchored) verify that the target range
      // actually contains what the caller expects before deleting it — a
      // silent-wrong-delete risk structurally identical to the pre-B38
      // str_replace bug, except here there was never a matching step to have
      // a bug in. This check is opt-in (only runs when expectedContent is
      // passed) so it doesn't break existing positional callers that don't
      // use it — same shape as insert's onLine+expectedContent pattern.
      if (expectedContent !== undefined && (_resolvedVia === 'positional' || _resolvedVia === 'afterString+endLine')) {
      ...
      const _isPositional = _resolvedVia === 'positional' || _resolvedVia === 'afterString+endLine';
      if (_isPositional) {
        try {
        ...
        ? '\n⚡ Line numbers used here are positional — they drift if lines are inserted or deleted above them.' +
          (_noSymbolsHere
            ? '\n   This file has no parseable function/symbol table (unsupported file type or parse failure) — inFunction won\'t work here.' +
              '\n   Prefer betweenHint:{start,end}, matchString, or afterString (for the whole range) on the next edit to this area.'
            : '\n   Prefer inFunction, startContent+endContent, matchString, betweenHint, or afterString (for the whole range) on the next edit to this area.')
        : '';
  ...
  async function readHandler({
    ...
    if (pairStart && pairEnd) {
      ...
      if (!_startMatch.matched && _startMatch.reason !== 'ambiguous') {
      ...
      const scope = resolveScope({
        ...
        params: { inFunction, afterFunction, beforeFunction, afterSymbol, beforeSymbol,
                  afterString, beforeString, afterLine, beforeLine, occurrence, fuzzyWhitespace },
      });
      if (scope.error) {
        bump('read', 'fails.anchorNotFound');
        const e = scope.error;
        if (e.kind === 'ambiguous') {
      ...
      if (!scope._hasScope) {
        throw new Error("Must provide startLine+endLine, centerLine/nearLine, inFunction, afterFunction, beforeFunction, afterSymbol, beforeSymbol, afterString, beforeString, afterLine, beforeLine, or an anchor pair (betweenHint or startContent+endContent).");
      }
...
// ── sed (migrated to Tool Framework) ─────────────────────────────────────
  ...
  if (g('edit')) registerMcpTool({
    ...
    description: [
      ...
      "TYPICAL WORKFLOW: call get-structural-anchors → pick a name → pass it to insert (sectionHint/preprocBlock) or delete-block (sectionHint/preprocBlock)."
    ].join(" "),
    inputSchema: {
      ...FILE_SCHEMA
    },
