...
// applyStyleCheck(newStr, filePath) — run style checker, return inline suffix string.
// ---------------------------------------------------------------------------
...
function applyStyleCheck(newStr, filePath) {
  ...
  try {
    ...
    console.log(`[style-check] _totalCHEdits=${styleStats._totalCHEdits} totalViolations=${totalViolations}`);
...
// Legacy shim — kept so existing call sites compile; delegates to smartSuggestion
function failureSuggestion(counter, toolName) {
  return smartSuggestion({ toolName, counter, noHintsUsed: true, fileLines: 0, oldStr: null, isCodeFile: false });
}
...
export function mcpRegistration(server, linterRegistry = null, getMessages = null, groups = {}) {
  ...
  if (g('edit')) {
  {
    ...
    server.registerTool(
      ...
      async ({ old_str, new_str, functionHint, lineHint, afterHint, betweenHint, occurrence = 1, fuzzyWhitespace = false, dryRun = false, lint = false }) => {
        ...
        const buffer = editor.getBuffer();
        const isCodeFile = /\.(js|ts|jsx|tsx|c|cpp|h|cs|py|java|go|rs)$/i.test(editor.getPath() || "");
          ...
          if (!fn) {
            ...
            return {
              content: [{ type: "text", text: [
                ...
                smartSuggestion({ toolName: curTool, counter: strReplFailures, noHintsUsed: false, fileLines: allLines.length, oldStr: old_str, isCodeFile: /\.(js|ts|jsx|tsx|c|cpp|h|cs|py|java|go|rs)$/i.test(editor.getPath()||"") })
              ].filter(Boolean).join("\n") }],
              matched: false
            };
          }
        ...
        const _lintSuffix = lint ? await (async () => {
          const endRow = matchLine + new_str.split("\n").length - 1;
...

  {
    ...
    server.registerTool(
      ...
      async ({ insert_line, new_str, afterContent, beforeContent, functionHint, functionEnd, occurrence = 1, dryRun = false, lint = false }) => {
        ...
        if (functionEnd !== undefined) {
          ...
          const _lintSuffix = lint ? await (async () => { const snap = await lintSnapshot(editor, insertRow, insertRow + new_str.split("\n").length - 1); return snap ? `\n${snap}` : ""; })() : "";
          const _styleSuffix = applyStyleCheck(new_str, editor.getPath() || "");
          return {
        ...
        if (afterContent !== undefined || beforeContent !== undefined) {
          ...
          if (!hit) {
            ...
            return { content: [{ type: "text", text: [
              ...
              smartSuggestion({ toolName: "insert", counter: insertFailures, noHintsUsed: !functionHint && !afterContent && !beforeContent, fileLines: allLines.length, oldStr: null, isCodeFile: /\.(js|ts|jsx|tsx|c|cpp|h|cs|py|java|go|rs)$/i.test(editor.getPath()||"") })
          ...
          bump('insert', 'hits');
          const newLineCount = buffer.getLineCount();
          const _lintSuffix2 = lint ? await (async () => { const snap = await lintSnapshot(editor, insertRow, insertRow + new_str.split("\n").length - 1); return snap ? `\n${snap}` : ""; })() : "";
        ...
        if (insert_line < 1 || insert_line > lineCount + 1) {
          ...
          return { content: [{ type: "text", text: [
            ...
            `\n📍 End of file:\n${ctx}`,
            smartSuggestion({ toolName: "insert", counter: insertFailures, noHintsUsed: true, fileLines: allLines.length, oldStr: null, isCodeFile: /\.(js|ts|jsx|tsx|c|cpp|h|cs|py|java|go|rs)$/i.test(editor.getPath()||"") })
          ].filter(Boolean).join("\n") }], inserted: false, insertFailures: insertFailures.count };
        }
        ...
        const newLineCount = buffer.getLineCount();
        const _lintSuffix3 = lint ? await (async () => { const snap = await lintSnapshot(editor, row, row + new_str.split("\n").length - 1); return snap ? `\n${snap}` : ""; })() : "";
      ...
      async ({ startLine, endLine, dryRun = false, functionHint, afterHint, lineHint, betweenHint, occurrence = 1, fuzzyWhitespace, lint = false }) => {
        ...
        if (startLine < 1 || endLine < 1) {
          ...
          return { content: [{ type: "text", text: [
            ...
            smartSuggestion({ toolName: "delete-line-range", counter: deleteFailures, noHintsUsed: !functionHint && !afterHint && !betweenHint && !lineHint, fileLines: lineCount, oldStr: null, isCodeFile: /\.(js|ts|jsx|tsx|c|cpp|h|cs|py|java|go|rs)$/i.test(editor.getPath()||"") })
        ...
        const newLineCount = buffer.getLineCount();
        const _lintSuffix = lint ? await (async () => { const snap = await lintSnapshot(editor, startRow, startRow + 5); return snap ? `\n${snap}` : ""; })() : "";
        return {
  ...
  // ── P6: delete-block ─────────────────────────────────────────────────────
  {
    ...
    server.registerTool(
      ...
      async ({ startContent, endContent, sectionHint, preprocBlock, preprocSide, inclusive = true, functionHint, occurrence = 1, dryRun = false, lint = false }) => {
        ...
        if (sectionHint !== undefined || preprocBlock !== undefined) {
          ...
          const newLineCount = buffer.getLineCount();
          const _lintSuffix = lint ? await (async () => { const snap = await lintSnapshot(editor, delStart, delStart + 5); return snap ? `\n${snap}` : ""; })() : "";
          return {
        ...
        if (startContent) {
          const _isCode = /\.(js|ts|jsx|tsx|c|cpp|h|cs|py|java|go|rs)$/i.test(editor.getPath() || "");
          ...
          if (_ambig) { bump('delete_block', 'fails.startNotFound'); deleteFailures.count++; return _ambig; }
        }
        ...
        bump('delete_block', 'hits');
        const newLineCount = buffer.getLineCount();
        const _lintSuffix2 = lint ? await (async () => { const snap = await lintSnapshot(editor, delStart, delStart + 5); return snap ? `\n${snap}` : ""; })() : "";
        return {
  ...
  // ── P7: replace-block ─────────────────────────────────────────────────────
  {
    ...
    server.registerTool(
      ...
      async ({ anchor, newBody, occurrence = 1, dryRun = false, functionHint, afterHint, lineHint, betweenHint, fuzzyWhitespace, lint = false }) => {
        ...
        const newLineCount = buffer.getLineCount();
        const _lintSuffix = lint ? await (async () => {
          const endRow2 = startRow + newBody.split("\n").length - 1;
          const snap = await lintSnapshot(editor, startRow, endRow2);
          return snap ? `\n${snap}` : "";
        })() : "";
  ...
  if (g('edit')) {
  {
    ...
    server.registerTool(curTool,
      ...
      async ({ query, replacement, regex = false, caseSensitive = false, dryRun = false, lint = false }) => {
        ...
        const _lintSuffix = lint ? await (async () => { const snap = await lintSnapshot(editor, null, null); return snap ? `\n${snap}` : ""; })() : "";
        return {
          content: [{ type: "text", text: `✅ Replaced all ${matchCount} occurrence${matchCount === 1 ? "" : "s"} of ${JSON.stringify(query)}.${_lintSuffix}` }],
    ...
    server.registerTool(curTool,
      ...
      async ({ expression, functionHint, dryRun = false, lint = false }) => {
        ...
        if (matchCount === 0) {
          ...
          parts.push(smartSuggestion({ toolName: "sed", counter: sedFailures, noHintsUsed: !functionHint, fileLines: lineCount, oldStr: expression, isCodeFile: /\.(js|ts|jsx|tsx|c|cpp|h|cs|py|java|go|rs)$/i.test(editor.getPath()||"") }));
        ...
        if (result === false) {
          ...
          if (hunks.length === 0) {
            return { content: [{ type: "text", text: [
              ...
              smartSuggestion({ toolName: "apply-patch", counter: patchFailures, noHintsUsed: true, fileLines: allLines.length, oldStr: null, isCodeFile: /\.(js|ts|jsx|tsx|c|cpp|h|cs|py|java|go|rs)$/i.test(editor.getPath()||"") })
          ...
          parts.push(smartSuggestion({ toolName: "apply-patch", counter: patchFailures, noHintsUsed: true, fileLines: allLines.length, oldStr: null, isCodeFile: /\.(js|ts|jsx|tsx|c|cpp|h|cs|py|java|go|rs)$/i.test(editor.getPath()||"") }));
    ...
    server.registerTool(curTool,
      ...
      async ({ reset = false }) => {
        ...
        function summarise(stats) {
          ...
          const editFails = Object.values(sr.fails).reduce((a, b) => a + b, 0)
            ...
            + Object.values((stats.delete_block||{fails:{}}).fails||{}).reduce((a,b)=>a+b,0)
            + Object.values((stats.sed||{fails:{}}).fails||{}).reduce((a,b)=>a+b,0);
          const editTotal = editHits + editFails;
          ...
          return {
            editHits, editFails, editTotal, editPct, searchHits, searchMisses, searchTotal,
            editSummary:   `${editTotal} edit ops: ${editHits} hits (${editPct}%), ${editFails} fails`,
            searchSummary: `${searchTotal} searches: ${searchHits} hits, ${searchMisses} misses`,
          };
        }
        ...
        function buildReport(stats, label) {
          ...
          const { editSummary, searchSummary } = summarise(stats);
          return {
            ...
            grep_file:            { hits: (stats.grep_file||{hits:0}).hits,
              missTotal: Object.values((stats.grep_file||{fails:{}}).fails||{}).reduce((a,b)=>a+b,0),
              misses: {...((stats.grep_file||{fails:{}}).fails||{})},
              hintsUsed: {...((stats.grep_file||{hintsUsed:{}}).hintsUsed||{})} },
            grep_project:         { hits: (stats.grep_project||{hits:0}).hits,
              missTotal: Object.values((stats.grep_project||{fails:{}}).fails||{}).reduce((a,b)=>a+b,0),
              misses: {...((stats.grep_project||{fails:{}}).fails||{})},
              hintsUsed: {...((stats.grep_project||{hintsUsed:{}}).hintsUsed||{})} },
            search_symbol:        { hits: (stats.search_symbol||{hits:0}).hits,
              missTotal: Object.values((stats.search_symbol||{fails:{}}).fails||{}).reduce((a,b)=>a+b,0),
              misses: {...((stats.search_symbol||{fails:{}}).fails||{})},
              hintsUsed: {...((stats.search_symbol||{hintsUsed:{}}).hintsUsed||{})} },
            find_text:            { hits: (stats.find_text||{hits:0}).hits,
              missTotal: Object.values((stats.find_text||{fails:{}}).fails||{}).reduce((a,b)=>a+b,0),
              misses: {...((stats.find_text||{fails:{}}).fails||{})},
              dryRuns: (stats.find_text||{dryRuns:0}).dryRuns },
            replace_document:     { hits: (stats.replace_document||{hits:0}).hits },
            replace_across_files: { hits: (stats.replace_across_files||{hits:0}).hits,
              failTotal: Object.values((stats.replace_across_files||{fails:{}}).fails||{}).reduce((a,b)=>a+b,0),
              fails: {...((stats.replace_across_files||{fails:{}}).fails||{})},
              dryRuns: (stats.replace_across_files||{dryRuns:0}).dryRuns },
          };
        }
        ...
        function buildStyleReport(ss, label) {
          ...
          // Build byRule from per-rule { introduced } objects
          const byRule = {};
        ...
        if (reset) {
          ...
          const STYLE_RULE_KEYS = [
            ...
            'operator_spacing','pointer_spacing','brace_placement','single_line_comment','consecutive_blanks'
          ];
          STYLE_RULE_KEYS.forEach(k => { if (styleStats[k]) styleStats[k].introduced = 0; });
        }
    ...
    server.registerTool(curTool,
      ...
      async ({ filePath }) => {
        ...
        try {
          ...
          const { violations, totalViolations } = styleCheckLines(content, targetPath);
...
export function getEditStats() {
  ...
  const avg = sr.hits > 0 ? Math.round((sr._oldStrLenSum / sr.hits) * 10) / 10 : 0;
  const editHits  = sr.hits + editStats.insert.hits + editStats.delete_line_range.hits
    + editStats.replace_function_body.hits + editStats.replace_block.hits
    + editStats.apply_patch.hits + editStats.replace_all.hits
    + editStats.replace_document.hits + editStats.replace_across_files.hits
    + editStats.delete_block.hits + editStats.sed.hits;
  const editFails = Object.values(sr.fails).reduce((a,b)=>a+b,0)
    ...
    + editStats.replace_across_files.fails.skipped
    + Object.values(editStats.delete_block.fails).reduce((a,b)=>a+b,0)
    + Object.values(editStats.sed.fails).reduce((a,b)=>a+b,0);
  const editTotal = editHits + editFails;
  const editPct   = editTotal > 0 ? Math.round((editHits / editTotal) * 100) : 100;
  const searchHits   = editStats.grep_file.hits + editStats.grep_project.hits
    + editStats.search_symbol.hits + editStats.find_text.hits;
  const searchMisses = editStats.grep_file.fails.noMatch + editStats.grep_project.fails.noMatch
    + editStats.search_symbol.fails.noMatch + editStats.find_text.fails.noMatch;
  const searchTotal = searchHits + searchMisses;

  // lifetime summary
  const lsr = lifetimeStats.str_replace;
  const lEditHits  = lsr.hits + lifetimeStats.insert.hits + lifetimeStats.delete_line_range.hits
    ...
    + (lifetimeStats.replace_document||{hits:0}).hits
    + (lifetimeStats.replace_across_files||{hits:0}).hits
    + (lifetimeStats.delete_block||{hits:0}).hits
    + (lifetimeStats.sed||{hits:0}).hits;
  const lEditFails = Object.values(lsr.fails).reduce((a,b)=>a+b,0)
    ...
    + ((lifetimeStats.replace_across_files||{fails:{skipped:0}}).fails||{skipped:0}).skipped
    + Object.values((lifetimeStats.delete_block||{fails:{}}).fails||{}).reduce((a,b)=>a+b,0)
    + Object.values((lifetimeStats.sed||{fails:{}}).fails||{}).reduce((a,b)=>a+b,0);
  const lEditTotal = lEditHits + lEditFails;
  const lEditPct   = lEditTotal > 0 ? Math.round((lEditHits / lEditTotal) * 100) : 100;
  const lSearchHits   = (lifetimeStats.grep_file||{hits:0}).hits + (lifetimeStats.grep_project||{hits:0}).hits
    + (lifetimeStats.search_symbol||{hits:0}).hits + (lifetimeStats.find_text||{hits:0}).hits;
  const lSearchMisses = ((lifetimeStats.grep_file||{fails:{noMatch:0}}).fails||{noMatch:0}).noMatch
    + ((lifetimeStats.grep_project||{fails:{noMatch:0}}).fails||{noMatch:0}).noMatch
    + ((lifetimeStats.search_symbol||{fails:{noMatch:0}}).fails||{noMatch:0}).noMatch
    + ((lifetimeStats.find_text||{fails:{noMatch:0}}).fails||{noMatch:0}).noMatch;
  const lSearchTotal = lSearchHits + lSearchMisses;

  const safeTool = (s, tool) => (s[tool] || { hits:0, fails:{}, hintsUsed:{}, dryRuns:0 });

  return {
    session: {
      sessionEditSummary:   `${editTotal} edit ops: ${editHits} hits (${editPct}%), ${editFails} fails`,
      sessionSearchSummary: `${searchTotal} searches: ${searchHits} hits, ${searchMisses} misses`,
      ...
      get_selection:         { hits: editStats.get_selection.hits, dryRuns: editStats.get_selection.dryRuns },
      get_linter_messages:     { hits: editStats.get_linter_messages.hits, dryRuns: editStats.get_linter_messages.dryRuns },
      find_text:               { hits: editStats.find_text.hits, missTotal: editStats.find_text.fails.noMatch, misses: {...editStats.find_text.fails}, dryRuns: editStats.find_text.dryRuns },
      grep_file:               { hits: editStats.grep_file.hits, missTotal: editStats.grep_file.fails.noMatch, misses: {...editStats.grep_file.fails}, hintsUsed: {...editStats.grep_file.hintsUsed} },
      grep_project:            { hits: editStats.grep_project.hits, missTotal: editStats.grep_project.fails.noMatch, misses: {...editStats.grep_project.fails}, hintsUsed: {...editStats.grep_project.hintsUsed} },
      search_symbol:           { hits: editStats.search_symbol.hits, missTotal: editStats.search_symbol.fails.noMatch, misses: {...editStats.search_symbol.fails}, hintsUsed: {...editStats.search_symbol.hintsUsed} },
      replace_document:        { hits: editStats.replace_document.hits },
      replace_across_files:    { hits: editStats.replace_across_files.hits, failTotal: editStats.replace_across_files.fails.skipped, fails: {...editStats.replace_across_files.fails}, dryRuns: editStats.replace_across_files.dryRuns },
    },
    lifetime: {
      lifetimeEditSummary:   `${lEditTotal} edit ops across ${lifetimeStats.sessionCount || 0} sessions: ${lEditHits} hits (${lEditPct}%), ${lEditFails} fails`,
      lifetimeSearchSummary: `${lSearchTotal} searches across ${lifetimeStats.sessionCount || 0} sessions: ${lSearchHits} hits, ${lSearchMisses} misses`,
      ...
      get_selection:         { hits: safeTool(lifetimeStats,'get_selection').hits, dryRuns: safeTool(lifetimeStats,'get_selection').dryRuns },
      ...
      find_text:               { hits: safeTool(lifetimeStats,'find_text').hits, missTotal: (safeTool(lifetimeStats,'find_text').fails||{noMatch:0}).noMatch, misses: {...(safeTool(lifetimeStats,'find_text').fails||{})}, dryRuns: safeTool(lifetimeStats,'find_text').dryRuns },
      grep_file:               { hits: safeTool(lifetimeStats,'grep_file').hits, missTotal: (safeTool(lifetimeStats,'grep_file').fails||{noMatch:0}).noMatch, misses: {...(safeTool(lifetimeStats,'grep_file').fails||{})}, hintsUsed: {...(safeTool(lifetimeStats,'grep_file').hintsUsed||{})} },
      grep_project:            { hits: safeTool(lifetimeStats,'grep_project').hits, missTotal: (safeTool(lifetimeStats,'grep_project').fails||{noMatch:0}).noMatch, misses: {...(safeTool(lifetimeStats,'grep_project').fails||{})}, hintsUsed: {...(safeTool(lifetimeStats,'grep_project').hintsUsed||{})} },
      search_symbol:           { hits: safeTool(lifetimeStats,'search_symbol').hits, missTotal: (safeTool(lifetimeStats,'search_symbol').fails||{noMatch:0}).noMatch, misses: {...(safeTool(lifetimeStats,'search_symbol').fails||{})}, hintsUsed: {...(safeTool(lifetimeStats,'search_symbol').hintsUsed||{})} },
      replace_document:        { hits: safeTool(lifetimeStats,'replace_document').hits },
      replace_across_files:    { hits: safeTool(lifetimeStats,'replace_across_files').hits, failTotal: (safeTool(lifetimeStats,'replace_across_files').fails||{skipped:0}).skipped, fails: {...(safeTool(lifetimeStats,'replace_across_files').fails||{})}, dryRuns: safeTool(lifetimeStats,'replace_across_files').dryRuns },
    },
  };
}
