...
const { checkNaming, checkFunctionDocs, buildDocSkeleton, formatNamingViolations, FN_DEF_RE: FN_DEF_RE_REG } = require('./naming-checker');
...
export function mcpRegistration(server, linterRegistry = null, getMessages = null, groups = {}, chatPanel = null) {
  ...
  // -------------------------------------------------------------------------
  {
    ...
    async ({ functionName, filePath, line: hintLine }) => {
      ...
      /* find the function definition line — use hint if provided */
      let fnLine = -1;
      let fnSignature = '';
      if (hintLine !== undefined) {
        /* line param is 1-based from check-function-docs output */
        const idx = hintLine - 1;
        if (idx >= 0 && idx < lines.length) {
          const m = lines[idx].match(FN_DEF_RE_REG);
          if (m && m[2] === functionName) { fnLine = idx; fnSignature = lines[idx]; }
        }
      }
      if (fnLine === -1) {
        /* fallback: scan the whole file */
        for (let i = 0; i < lines.length; i++) {
          const m = lines[i].match(FN_DEF_RE_REG);
          if (m && m[2] === functionName) { fnLine = i; fnSignature = lines[i]; break; }
        }
      }
      if (fnLine === -1) {
