...
// ---------------------------------------------------------------------------
function anchorError(hintName, hintValue, result) {
  if (result === null) {
    return `❌ ${hintName} "${hintValue}" not found — no matching symbol, line number, or string in file.`;
  }
  if (result.ambiguous) {
...
const ANCHOR_SCHEMA = {
  ...
  fuzzyContent:    z.boolean().optional(),  // match ignoring smart-quotes, BOM, zero-width chars, emoji
...
function smartSuggestion(ctx) {
  ...
  if (noHintsUsed) {
    if (toolName === "str_replace" || toolName === "replace-block") {
      ...
      parts.push("   fuzzyContent:true     → retry ignoring smart-quotes, BOM, zero-width chars, emoji");
      parts.push("   regex:true            → treat old_str as a JS regex pattern to wildcard over problematic chars");
...
export const TOOL_CATALOGUE = [
  ...
  // Edit
  { name: "str_replace",             group: "edit",        desc: "Replace old_str with new_str. DECISION: know the function name? → functionHint. Know nearby unique text? → afterHint or betweenHint. Have a line number? → lineHint. Same pattern appears N times? → occurrence:N. Whitespace mismatch? → fuzzyWhitespace:true. Smart-quotes/emoji/BOM mismatch? → fuzzyContent:true. Backticks/pipes/special chars? → regex:true to wildcard over them. Unsure of match? → dryRun:true first." },
...
export function mcpRegistration(server, linterRegistry = null, getMessages = null, groups = {}, chatPanel = null) {
  ...
  if (g('edit')) {
  {
    ...
    server.registerTool(
      ...
      async ({ old_str, new_str, functionHint, lineHint, afterHint, betweenHint, occurrence = 1, fuzzyWhitespace = false, fuzzyContent = false, regex = false, dryRun = false, lint = false }) => {
        console.log(`CMD: ${curTool}, ARGS:`, { old_str: old_str.substring(0, 80), new_str: new_str.substring(0, 80), functionHint, lineHint, afterHint, betweenHint, occurrence, fuzzyWhitespace, fuzzyContent, regex, dryRun });
        ...
        // chars, and emoji in both old_str and buffer text for comparison only,
        ...
        if (fuzzyContent) {
          ...
          if (idx !== -1) {
            ...
            effectiveOldStr = searchText.substring(idx, idx + effectiveOldStr.length);
          }
          // If still no match, fall through to normal search (will fail and report)
        }
        ...
        // ── Layer 2: lineHint fallback — delete+insert when lineHint is confirmed ──
        // Both exact and fuzzy match failed. If lineHint was provided, the LLM
        // believes old_str starts at that line. Use it as a positional anchor:
        // delete the same number of lines as old_str and insert new_str there.
        // Character encoding mismatches become irrelevant — we trust the line number.
        if (lineHint && lineHint >= 1 && lineHint <= allLines.length) {
          const fallbackRow   = lineHint - 1;  // 0-based
          const oldLines      = old_str.split('\n');
          const deleteCount   = oldLines.length;
          const endRow        = Math.min(fallbackRow + deleteCount - 1, allLines.length - 1);
          const endCol        = allLines[endRow] ? allLines[endRow].length : 0;
          const startPos      = [fallbackRow, 0];
          ...
          strReplFailures.count = 0;
          bump('str_replace', 'hits');
          bump('str_replace', 'lineHintFallback');
          bump('str_replace', '_oldStrLenSum', deleteCount);
          const _lintSuffix  = await maybeLintSuffix(lint, editor, fallbackRow, fallbackRow + new_str.split('\n').length - 1);
          const _styleSuffix = applyStyleCheck(new_str, editor.getPath());
          return {
            content: [{ type: 'text', text: `✅ Replaced lines ${fallbackRow + 1}–${endRow + 1} via lineHint fallback (text match failed, line-range delete+insert used). [lineHintFallback]${_lintSuffix}${_styleSuffix}` }],
            matched: true,
            dryRun: false,
            replacedAtLine: fallbackRow + 1,
          };
        }
        ...
        const _styleSuffix = applyStyleCheck(new_str, editor.getPath());
        return {
          content: [{ type: "text", text: `✅ Replaced match at line ${matchLine + 1}${scopeLabel}${occurrence > 1 ? ` (occurrence ${occurrence})` : ""}${fuzzyWhitespace ? " [fuzzyWhitespace]" : ""}${fuzzyContent ? " [fuzzyContent]" : ""}${regex ? " [regex]" : ""}.${_nudge}${_lintSuffix}${_styleSuffix}` }],
          matched: true,
          dryRun: false,
...

  {
    ...
    server.registerTool(curTool,
      ...
      async ({ name, newBody, dryRun = false, occurrence = 1, functionHint, afterHint, lineHint, betweenHint, fuzzyWhitespace, lint = false }) => {
        ...
        if (lineHint)     bump('replace_function_body', 'hintsUsed.lineHint');
        ...
        // lineHint: convert 1-based to 0-based for proximity search
        const lineHintRow = lineHint != null ? Math.max(0, lineHint - 1) : undefined;
        ...
        const found = findFunction(scopedSymbols, name, {
          occurrence,
          lineHint:    lineHintRow,
          afterRow,
          betweenRows,
        });
        ...
        // ── Ambiguity warning — multiple definitions, no scoping hint ────────
        {
          const _noScope = !functionHint && !afterHint && !betweenHint && !lineHint && occurrence <= 1;
          if (_noScope) {
            const dupes = allSymbols.filter(s => s.name === name);
            if (dupes.length > 1) {
              const lineNums = dupes.map(s => s.startRow + 1).join(", ");
              bump('replace_function_body', 'fails.ambiguous');
              return { content: [{ type: "text", text: [
                `⚠️  AMBIGUOUS — "${name}" has ${dupes.length} definitions in file (lines: ${lineNums}).`,
                `   Use occurrence:N, lineHint, afterHint, or betweenHint to target one:`,
                `   • occurrence:N   → Nth definition (1=line ${dupes[0].startRow + 1}, 2=line ${dupes[1]?.startRow + 1 ?? "?"}, ...)`,
                `   • lineHint:N     → picks definition closest to line N`,
      ...
      async ({ reset = false }) => {
        ...
        if (reset) {
          ...
          for (const [, val] of Object.entries(editStats)) {
            ...
            if (typeof val.regexCommits            === 'number') val.regexCommits            = 0;
            if (typeof val.lineHintFallback        === 'number') val.lineHintFallback        = 0;
  ...
  // -------------------------------------------------------------------------
  {
    ...
    async ({ functionName, filePath, line: hintLine }) => {
      ...
      const _docMatch = findFunction(_docSyms, functionName, hintLine !== undefined ? { lineHint: hintLine } : {});
      if (_docMatch) {
...
function buildReport(stats, label) {
  ...
  return {
    ...
    str_replace: {
      ...
      regexCommits:           sr.regexCommits || 0,
      lineHintFallback:       sr.lineHintFallback || 0,
