...
function _failureSuggestion(counter, toolName) {
    ...
    server.registerTool(curTool,
      ...
      async ({ command, cwd, timeout = 30000, confirm = false, showOutput = true }) => {
        ...
        return new Promise((resolve) => {
          ...
          // On Windows PowerShell needs -Command flag; on Unix /bin/sh needs -c.
          const spawnArgs = IS_WINDOWS ? ['-Command', command] : ['-c', command];
...
export function getEditStats() {
  ...
  const searchHits   = editStats.grep_file.hits + editStats.grep_project.hits
    ...
    + (editStats.get_repo_map||{hits:0}).hits;
  ...
  const lSearchHits   = (lifetimeStats.grep_file||{hits:0}).hits + (lifetimeStats.grep_project||{hits:0}).hits
    ...
    + (lifetimeStats.get_repo_map||{hits:0}).hits;
  ...
  return {
    ...
    session: {
      ...
      get_repo_map:            { hits: (editStats.get_repo_map||{hits:0}).hits, fails: {...((editStats.get_repo_map||{}).fails||{})} },
      run_command:             { hits: (editStats.run_command||{hits:0}).hits, fails: {...((editStats.run_command||{}).fails||{})} },
    ...
    lifetime: {
      ...
      get_repo_map:            { hits: safeTool(lifetimeStats,'get_repo_map').hits, fails: {...(safeTool(lifetimeStats,'get_repo_map').fails||{})} },
