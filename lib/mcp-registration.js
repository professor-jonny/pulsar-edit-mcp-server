...
const { getSymbols, getSymbolsFromText, findFunction, resolveAnchor, braceEndRow: _tssBraceEndRow } = require('./tree-sitter-symbols'); // v2
...
function anchorError(hintName, hintValue, result, symbols) {
 ...
 // module scope, survive across tool calls,
...
// Append one NDJSON line to failure-log.ndjson -- zero deps, grep-queryable.
// entry shape: { ts, tool, reason, filePath, lineNumberHint?, hintsSet, oldStrPreview, diffVsBuffer? }
...
const ANCHOR_SCHEMA = {
  ...
  lineNumberHint:        z.number().optional(),   // start search at or after line N
  ...
  fuzzyContent:    z.boolean().optional(),  // match ignoring smart-quotes, BOM, zero-width chars, box-drawing runs, emoji
  regex:           z.boolean().optional(),  // treat old_str as a JS regex pattern
...
// noScopeHint   — true when no functionHint/afterHint/betweenHint/lineNumberHint and occurrence<=1
...
const TOOL_CATALOGUE = [
  ...
  // Edit
  { name: "str_replace",             group: "edit",        desc: "Replace old_str with new_str. DECISION: know the function name? \u2192 functionHint. Know nearby unique text? \u2192 afterHint or betweenHint. Have a line number? \u2192 lineNumberHint (positional) or lineContentHint:'text on that line' (content-stable, drift-immune). Same pattern appears N times? \u2192 occurrence:N. " +
  ...
  { name: "insert",                  group: "edit",        desc: "Insert new lines into the active editor without touching existing content. Use afterContent/beforeContent (content anchors — immune to line drift) rather than insert_line. functionEnd inserts after a named function's closing brace. dryRun previews position before writing. RESPONSE: ✅ insert — line N, +M lines [scopeLabel]. Followed by ⚠️ style/lint warnings if any issues in the inserted region." },
  ...
  { name: "delete-line-range",       group: "edit",        desc: "Delete a contiguous block of lines. functionHint deletes the entire named function body. betweenHint:{start,end} resolves both boundaries from content anchors. afterHint resolves the start line from content. dryRun previews what will be removed. RESPONSE: ✅ delete-line-range — line N, -M lines [tags]. Followed by ⚠️ lint: if linter fires near the deletion point." },
  ...
  { name: "replace-function-body",   group: "fileOps",     desc: "Replace a named function's entire signature + body atomically. PREFERRED over str_replace for whole-function rewrites — no risk of partial match or line shift. Use afterHint/lineNumberHint/occurrence to disambiguate if the name appears in multiple places. dryRun previews. RESPONSE: ✅ replace-function-body — line N, ±M lines \"fnName\". Followed by ⚠️ style/lint warnings — act on these before your next edit." },
  { name: "replace-across-files",    group: "fileOps",     desc: "Find and replace across all project files. When regex:true, replacement supports $1/$2/$& capture-group backreferences (same as VS Code find/replace). First call without confirm returns a preview. Call again with confirm:true to commit. maxMatches cap (default 50) prevents accidental mass edits." },
    ...
    server.registerTool(
      curTool,
      {
        ...
        inputSchema: {
          ...
          lineContentHint: z.string().optional(),
      ...
      async ({ old_str, new_str, functionHint, lineNumberHint, lineContentHint, afterHint, betweenHint, occurrence = 1, fuzzyWhitespace = false, fuzzyContent = false, regex = false, dryRun = false, lint = false }) => {
        ...
        if (functionHint) {
          ...
          const LINE_HINT_RADIUS = 50;
          const lhStartRow = Math.max(0, lineNumberHint - 1);                          // 0-based, 1 before hint
          const lhEndRow   = Math.min(allLines.length - 1, lineNumberHint - 1 + LINE_HINT_RADIUS);
          ...
          const lchEndRow = Math.min(allLines.length - 1, lchRow + LINE_CONTENT_RADIUS);
          searchStart = allLines.slice(0, lchRow).reduce((s, l) => s + l.length + 1, 0);
          searchEnd   = allLines.slice(0, lchEndRow + 1).reduce((s, l) => s + l.length + 1, 0);
          scopeLabel  = ` after "${lineContentHint.substring(0, 40)}" (line ${lchRow + 1}, ±${LINE_CONTENT_RADIUS})`;
        }
        ...
        if (matchIndex === -1) {
          ...
          // 4. Levenshtein similarity — multi-line old_str only (single-line is
          //    already well-served by diffVsBuffer). Compare old_str against the
          //    same-length buffer slice at the closest matched region.
          if (lines.length >= 2) {
