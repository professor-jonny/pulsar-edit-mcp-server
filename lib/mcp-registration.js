...
const { walkDir, resolveStructuralAnchor, findAnchor, findFunctionInBuffer, readFileOrBuffer, retargetEditor } = require('./buffer-helpers');
...
const {
  anchorError, smartSuggestion, successNudge, ambiguityCheck,
  patchFailures, strReplFailures, insertFailures, deleteFailures,
  deleteBlockFailures, replaceBlockFailures,
} = require('./tool-hints');
const {
...
function mcpRegistration(server, linterRegistry = null, getMessages = null, groups = {}, chatPanel = null) {
    ...
    server.registerTool(
      curTool,
      {
        ...
        description: [
          ...
          "(4) Have a line number from grep-file or read-lines? → lineNumberHint:42 — starts search at that line (positional, may drift).",
          "(4b) Know a unique string on the anchor line but not a line number? → lineContentHint:'some text on that line' — content-stable, drift-immune.",
      ...
      async ({ old_str, new_str, functionHint, lineNumberHint, lineContentHint, afterHint, betweenHint, inFunction, inSymbol, afterFunction, beforeFunction, afterSymbol, beforeSymbol, afterString, beforeString, afterLine, beforeLine, occurrence = 1, fuzzyWhitespace = false, fuzzyContent = false, regex = false, dryRun = false, lint = false }) => {
        console.log(`CMD: ${curTool}, ARGS:`, { old_str: old_str.substring(0, 80), new_str: new_str.substring(0, 80), functionHint, lineNumberHint, lineContentHint, afterHint, betweenHint, occurrence, fuzzyWhitespace, fuzzyContent, regex, dryRun });
        ...
        const allSymbols = getSymbols(editor, text, editor.getPath());

        // ── Resolve search bounds ─────────────────────────────────────────────
        // Priority: functionHint > betweenHint > afterHint > lineNumberHint > entire file
        let searchStart = 0;
        ...
        if (functionHint) {
          ...
          if (!fn) {
            const knownFns = allSymbols.map(s => `  ${s.startRow + 1}: ${s.name}`).slice(0, 20);
          ...
          if (errStart) {
            strReplFailures.count++;
            bump('str_replace', 'fails.outOfScope');
            return { content: [{ type: "text", text: [errStart, smartSuggestion({ toolName: curTool, counter: strReplFailures, noHintsUsed: false, fileLines: allLines.length, oldStr: old_str, isCodeFile })].filter(Boolean).join("\n") }], matched: false };
          }
          ...
          if (errEnd) {
            strReplFailures.count++;
            bump('str_replace', 'fails.outOfScope');
            return { content: [{ type: "text", text: [errEnd, smartSuggestion({ toolName: curTool, counter: strReplFailures, noHintsUsed: false, fileLines: allLines.length, oldStr: old_str, isCodeFile })].filter(Boolean).join("\n") }], matched: false };
          }
          ...
          const r = resolveAnchor(afterHint, allSymbols, text);
          const err = anchorError('afterHint', afterHint, r, allSymbols);
          if (err) {
            strReplFailures.count++;
            bump('str_replace', 'fails.afterNotFound');
            return { content: [{ type: "text", text: [err, smartSuggestion({ toolName: curTool, counter: strReplFailures, noHintsUsed: false, fileLines: allLines.length, oldStr: old_str, isCodeFile })].filter(Boolean).join("\n") }], matched: false };
          }
          searchStart = buffer.characterIndexForPosition([r.row + 1, 0]);
          scopeLabel  = ` after "${afterHint.substring(0, 40)}" [${r.via}] (line ${r.row + 1})`;
        } else if (lineNumberHint) {
          const lhStartRow = Math.max(0, lineNumberHint - 1 - HINT_RADIUS);
          const lhEndRow   = Math.min(allLines.length - 1, lineNumberHint - 1 + HINT_RADIUS);
          searchStart = allLines.slice(0, lhStartRow).reduce((s, l) => s + l.length + 1, 0);
          searchEnd   = allLines.slice(0, lhEndRow + 1).reduce((s, l) => s + l.length + 1, 0);
          scopeLabel  = ` near line ${lineNumberHint} (±${HINT_RADIUS})`;
        } else if (lineContentHint) {
          // lineContentHint: content-stable line anchor — find the first line containing this string
          const lchRow = allLines.findIndex(l => l.includes(lineContentHint));
          if (lchRow === -1) {
            return { content: [{ type: 'text', text: `❌ lineContentHint "${lineContentHint.substring(0, 60)}" not found in file.` }] };
          }
          const lchStartRow = Math.max(0, lchRow - HINT_RADIUS);
          const lchEndRow   = Math.min(allLines.length - 1, lchRow + HINT_RADIUS);
          searchStart = allLines.slice(0, lchStartRow).reduce((s, l) => s + l.length + 1, 0);
          searchEnd   = allLines.slice(0, lchEndRow + 1).reduce((s, l) => s + l.length + 1, 0);
          scopeLabel  = ` after "${lineContentHint.substring(0, 40)}" (line ${lchRow + 1}, ±${HINT_RADIUS})`;
        // ── New hint family ───────────────────────────────────────────────────
        // inFunction / inSymbol — scope search to symbol body
        } else if (inFunction || inSymbol) {
          const hintName = inFunction ? 'inFunction' : 'inSymbol';
          const hintVal  = inFunction || inSymbol;
          const kind     = inFunction ? 'function' : 'any';
          const r = resolveSymbolPosition(allSymbols, hintVal, kind, 'inside');
          ...
          if (r.notFound) {
            const known = allSymbols.filter(s => kind === 'function' ? ['function','registerTool'].includes(s.kind) : true).map(s => `  ${s.startRow + 1}: ${s.name} [${s.kind}]`).slice(0, 20);
            strReplFailures.count++;
            bump('str_replace', 'fails.outOfScope');
            return { content: [{ type: 'text', text: [`❌ ${hintName}: "${hintVal}" not found.`, `Known symbols:\n${known.join('\n') || '  (none)'}`, smartSuggestion({ toolName: curTool, counter: strReplFailures, noHintsUsed: false, fileLines: allLines.length, oldStr: old_str, isCodeFile })].filter(Boolean).join('\n') }], matched: false };
          }
          if (r.ambiguous) {
            const list = r.matches.map(m => `  line ${m.startRow + 1}: ${m.name} [${m.kind}]`).join('\n');
            strReplFailures.count++;
            bump('str_replace', 'fails.outOfScope');
            return { content: [{ type: 'text', text: `❌ ${hintName}: "${hintVal}" is ambiguous — ${r.matches.length} matches:\n${list}\nUse occurrence:N to disambiguate.` }], matched: false };
          }
          searchStart = buffer.characterIndexForPosition([r.startRow, 0]);
          searchEnd   = buffer.characterIndexForPosition([r.endRow + 1, 0]);
          scopeLabel  = ` within ${hintName} "${hintVal}" (lines ${r.startRow + 1}–${r.endRow + 1})`;
        ...
        // afterFunction / beforeFunction — window relative to function symbol
          ...
          const r = resolveSymbolPosition(allSymbols, hintVal, 'function', dir);
          if (r.notFound) {
        ...
        // afterSymbol / beforeSymbol — window relative to any symbol
          ...
          const r = resolveSymbolPosition(allSymbols, hintVal, 'any', dir);
          if (r.needsTreeSitter) {
            strReplFailures.count++;
            bump('str_replace', 'fails.outOfScope');
            return { content: [{ type: 'text', text: `❌ ${hintName}: requires the file to be open in the editor with a grammar assigned (no tree-sitter parse tree found).` }], matched: false };
        ...
        // afterString / beforeString — window relative to a line containing text
          ...
          const r = resolveStringPosition(allLines, hintVal, dir);
          if (r.notFound) {
            strReplFailures.count++;
            bump('str_replace', 'fails.afterNotFound');
            return { content: [{ type: 'text', text: [`❌ ${hintName}: "${hintVal.substring(0, 60)}" not found in file.`, smartSuggestion({ toolName: curTool, counter: strReplFailures, noHintsUsed: false, fileLines: allLines.length, oldStr: old_str, isCodeFile })].filter(Boolean).join('\n') }], matched: false };
          }
          if (r.ambiguous) {
        ...
        // afterLine / beforeLine — positional last resort
          ...
          const r = resolveLinePosition(allLines.length, hintVal, dir);
          searchStart = buffer.characterIndexForPosition([r.startRow, 0]);
          searchEnd   = buffer.characterIndexForPosition([r.endRow + 1, 0]);
        ...
        if (inSymbol)              bump('str_replace', 'hintsUsed.inSymbol');
        ...
        if (lineNumberHint)              bump('str_replace', 'hintsUsed.lineNumberHint');
        if (lineContentHint)       bump('str_replace', 'hintsUsed.lineNumberHint'); // counted same bucket as lineNumberHint
        ...
        if (isMultiLine) {
          ...
          const startRow = lineNumberHint ? Math.max(0, lineNumberHint - 1) : 0;
        ...
        // -- lineNumberHint ambiguity guard --
        // When lineNumberHint was used and occurrence===1 (default), check whether
        // old_str matches more than once inside the ±RADIUS window. If so,
        // refuse and report all match locations so the caller can add occurrence:N.
        if (lineNumberHint && occurrence === 1 && matchIndex !== -1 && !regex) {
          const windowText = text.substring(searchStart, searchEnd);
          const windowStartLine = text.substring(0, searchStart).split('\n').length - 1; // 0-based
          let ambigCount = 0;
          const ambigMatches = []; // { line (1-based), preview }
          let scanFrom = 0;
          while (scanFrom < windowText.length) {
            const sidx = windowText.indexOf(effectiveOldStr, scanFrom);
            if (sidx === -1) break;
            ambigCount++;
            const absLine0 = windowStartLine + windowText.substring(0, sidx).split('\n').length - 1; // 0-based
            // Collect 1 line before + matched lines + 1 line after as preview
            const matchLineCount = effectiveOldStr.split('\n').length;
            const previewStart = Math.max(0, absLine0 - 1);
            const previewEnd   = Math.min(allLines.length - 1, absLine0 + matchLineCount);
            const preview = allLines.slice(previewStart, previewEnd + 1)
              .map((l, i) => {
                const ln = previewStart + i;
                const marker = (ln >= absLine0 && ln < absLine0 + matchLineCount) ? '▶' : ' ';
                return '  ' + marker + ' ' + (previewStart + i + 1) + ': ' + l;
              }).join('\n');
            ambigMatches.push({ line: absLine0 + 1, preview });
            scanFrom = sidx + 1;
            if (ambigCount > 9) { ambigMatches.push({ line: '…', preview: '' }); break; }
          }
          if (ambigCount > 1) {
            strReplFailures.count++;
            bump('str_replace', 'fails.ambiguous');
            const matchSections = ambigMatches
              .filter(m => m.preview)
              .map((m, i) => 'occurrence:' + (i + 1) + ' — line ' + m.line + ':\n' + m.preview)
              .join('\n\n');
            return {
              content: [{ type: 'text', text: [
                '❌ lineNumberHint ambiguity: old_str matched ' + ambigCount + ' time(s) within the ±50 line window around line ' + lineNumberHint + '.',
                '💡 Add occurrence:N to target the one you want, or use functionHint/betweenHint.\n',
                matchSections,
                smartSuggestion({ toolName: curTool, counter: strReplFailures, noHintsUsed: false, fileLines: allLines.length, oldStr: old_str, isCodeFile: isCodeFilePath(editor.getPath()) })
              ].filter(Boolean).join('\n') }],
              matched: false,
              ambigCount,
              ambigLines: ambigMatches.map(m => m.line),
              strReplFailures: strReplFailures.count
            };
          }
        }
        ...
        // ── Auto-retry: fuzzyWhitespace / fuzzyContent ───────────────────────
        // If no match and the flags were NOT explicitly set by the caller, try
        // them automatically. Track whether each was auto-triggered for tagging.
        let autoFuzzyWhitespace = false;
        let autoFuzzyContent    = false;

        if (matchIndex === -1 && !fuzzyWhitespace && !regex) {
          const searchText = text.substring(searchStart, searchEnd);
          const searchLines = searchText.split('\n');
          const needle = old_str.split('\n');
          let fuzzyMatchStart = -1;
          ...
          for (let i = 0; i <= searchLines.length - needle.length; i++) {
            for (let j = 0; j < needle.length; j++) {
              if (searchLines[i + j].trim() !== needle[j].trim()) continue outer_fw;
            }
            fuzzyMatchStart = i;
            break;
          }
          if (fuzzyMatchStart !== -1) {
            const rebuiltOldStr = searchLines.slice(fuzzyMatchStart, fuzzyMatchStart + needle.length).join('\n');
            ...
            if (autoIdx !== -1 && autoIdx < searchEnd) {
              ...
              fuzzyWhitespace     = true; // flag for tagging + stats
        ...
        // ── Auto-retry: fuzzyContent ──────────────────────────────────────────
        // If still no match and fuzzyContent was NOT explicitly set, try Unicode
        // normalisation automatically. Catches → arrows, smart-quotes, BOM etc.
        if (matchIndex === -1 && !fuzzyContent && !regex) {
          const normalise = s => s
            .replace(/\uFEFF/g, '')
            .replace(/[\u200B\u200C\u200D\u00AD]/g, '')
            .replace(/\u00A0/g, ' ')
            ...
            .replace(/[\u2500-\u257F]+/g, '--')
            .replace(/[\uD800-\uDBFF][\uDC00-\uDFFF]/g, '\u{1F4CC}');
          const normNeedle   = normalise(old_str);
          const searchText   = text.substring(searchStart, searchEnd);
          const normHaystack = normalise(searchText);
          const autoFCIdx    = normHaystack.indexOf(normNeedle);
          if (autoFCIdx !== -1) {
            // Walk raw searchText forward to find the raw span matching normNeedle
            let srcIdx   = autoFCIdx;
            let consumed = 0;
            while (consumed < normNeedle.length && srcIdx < searchText.length) {
              consumed += normalise(searchText[srcIdx]).length;
              srcIdx++;
            }
            const rebuiltOldStr = searchText.substring(autoFCIdx, srcIdx);
            const absIdx = searchStart + autoFCIdx;
            matchIndex = absIdx;
            matchLine  = text.substring(0, absIdx).split('\n').length - 1;
            effectiveOldStr = rebuiltOldStr;
            fuzzyContent     = true; // flag for tagging + stats
            autoFuzzyContent = true;
            bump('str_replace', 'fuzzyContentCommits');
          }
        }

        // ── Auto-retry: strip trailing comment ───────────────────────────────
        // If still no match, check if old_str's last line has a trailing comment
        // (/* */ or //) that the buffer line doesn't. Strip it, try matching the
        // comment-free prefix. On success, append the salvaged comment to
        // effectiveNewStr's last line as /* CHECK: <comment> */ so it isn't lost.
        ...
        if (matchIndex === -1 && !regex) {
          ...
          const commentMatch = lastLine.match(trailingCommentRe);
          if (commentMatch) {
            const strippedLast = lastLine.slice(0, lastLine.length - commentMatch[1].length);
            const stripped = oldLines.slice(0, -1).concat(strippedLast).join('\n');
            const searchText  = text.substring(searchStart, searchEnd);
            const searchLines = searchText.split('\n');
            const needle      = stripped.split('\n');
            let fuzzyMatchStart = -1;
            outer_sc:
            for (let i = 0; i <= searchLines.length - needle.length; i++) {
              for (let j = 0; j < needle.length; j++) {
                if (searchLines[i + j].trim() !== needle[j].trim()) continue outer_sc;
              }
              fuzzyMatchStart = i;
              break;
            }
            if (fuzzyMatchStart !== -1) {
              const rebuiltOldStr = searchLines
                .slice(fuzzyMatchStart, fuzzyMatchStart + needle.length).join('\n');
              const autoIdx = text.indexOf(rebuiltOldStr, searchStart);
              if (autoIdx !== -1 && autoIdx < searchEnd) {
                matchIndex       = autoIdx;
                matchLine        = text.substring(0, autoIdx).split('\n').length - 1;
                effectiveOldStr  = rebuiltOldStr;
                salvagedComment  = commentMatch[1].trim();
        ...
        // window (lineNumberHint has drifted), attempt a full match there with
        // fuzzyWhitespace normalisation. This rescues the most common partialMatch
        // cause: a prior insert/delete shifted the target region so lineNumberHint
        // or afterHint now points at the wrong area.
        ...
        if (matchIndex === -1) {
          ...
          const parts = [
            `❌ No match found for old_str${scopeLabel || (lineNumberHint ? ` at or after line ${lineNumberHint}` : "")}.`
          ];
          if (wsIssues.length > 0) {
          ...
          }
          {
            ...
            if (simPct >= 80) {
              ...
              simHint = 'Low match — old_str may be pointing at the wrong location entirely. Verify lineNumberHint or scope hints and re-read.';
            }
          ...
          const sugg = smartSuggestion({
            ...
            noHintsUsed: !functionHint && !afterHint && !betweenHint && !lineNumberHint && occurrence <= 1,
          ...
          // If lineNumberHint was provided, show a char-level diff of old_str's first
          // line vs what's actually at that position in the buffer -- the highest-
          // signal field for diagnosing why a match failed.
          let diffVsBuffer = null;
          if (lineNumberHint != null) {
            const hintRow  = lineNumberHint - 1; // 0-based
            const oldLines = old_str.split('\n');
            const bufSlice = allLines.slice(hintRow, hintRow + Math.min(oldLines.length, 4));
            if (bufSlice.length > 0) {
              const diffs = oldLines.slice(0, bufSlice.length).map((ol, i) => {
                const bl = bufSlice[i] ?? '';
                if (ol === bl) return null;
                return `  old[${i + 1}]: ${JSON.stringify(ol)}\n  buf[${i + 1}]: ${JSON.stringify(bl)}`;
              }).filter(Boolean);
              if (diffs.length > 0) {
                diffVsBuffer = diffs.join('\n');
                parts.push(`\n🔬 DIFF vs buffer at line ${lineNumberHint}:\n${diffVsBuffer}`);
              }
            }
          }
          {
            // Capture raw buffer lines around the hint for the fault log viewer.
            // Uses lineNumberHint if set, otherwise falls back to diagOffset (fuzzy match row).
            // ±5 lines, formatted as "L<n>: <content>" so old vs buffer is directly comparable.
            const _previewRow   = lineNumberHint != null ? lineNumberHint - 1 : diagOffset;
            ...
            logFailure({
              tool: curTool,
              reason: wsIssues.length > 0 ? 'whitespace' : partialMatchLines > 0 ? 'partialMatch' : 'noMatch',
              filePath: editor.getPath(),
              lineNumberHint: lineNumberHint ?? null,
              hintsSet: [functionHint && 'functionHint', afterHint && 'afterHint', betweenHint && 'betweenHint',
                         lineNumberHint && 'lineNumberHint', occurrence > 1 && 'occurrence'].filter(Boolean),
        ...
        const _ambig = ambiguityCheck({
          ...
          noScopeHint: !functionHint && !afterHint && !betweenHint && !lineNumberHint && occurrence <= 1,
        ...
        const _nudge = successNudge({
          toolName: curTool,
          noHintsUsed: !functionHint && !afterHint && !betweenHint && !lineNumberHint && !lineContentHint && occurrence <= 1,
          lineNumberHintOnly: (lineNumberHint || lineContentHint) && !functionHint && !afterHint && !betweenHint,
        ...
        const _styleSuffix = applyStyleCheck(effectiveNewStr, editor.getPath());
        const _tags = [
    ...
    server.registerTool(
      curTool,
      {
        ...
        description: [
          ...
          "occurrence:N — return only the Nth match with its context. Use this to get the exact line number of a specific instance, then pass that line number to str_replace lineNumberHint.",
          ...
          "Returns totalMatches so you know how many times the pattern appears " + String.fromCharCode(8212) + " if totalMatches > 1 you should use occurrence:N or a hint with str_replace to avoid hitting the wrong instance."
        ].join(" "),
        inputSchema: {
          query:         z.string(),
...

  {
    ...
    server.registerTool(
      ...
      async ({ text }) => {
        ...
        const _styleSuffix = applyStyleCheck(text, editor.getPath());
        const linesChanged = lines.length - originalText.split(/\r?\n/).length;
        return {
          ...buildEditResponse(
            { tool: curTool, line: 1, linesChanged, scopeLabel: '', tags: [] },
            { style: _styleSuffix }
          ),
    ...
    server.registerTool(
      curTool,
      {
        ...
        inputSchema: {
          ...
          afterFunction:   z.string().optional(),   // insert after named function's closing brace
          beforeFunction:  z.string().optional(),   // insert before named function's opening line
          afterSymbol:     z.string().optional(),   // insert after any named symbol's closing
          beforeSymbol:    z.string().optional(),   // insert before any named symbol's opening
          afterString:     z.string().optional(),   // insert after the line containing this text
          beforeString:    z.string().optional(),   // insert before the line containing this text
          ...STRUCTURAL_ANCHOR_SCHEMA,  // afterContent, beforeContent, functionEnd, sectionHint, preprocBlock, preprocSide, functionHint, occurrence, dryRun (kept for back-compat)
        }
      },
      async ({ insert_line, new_str, endOfFile = false, afterFunction, beforeFunction, afterSymbol, beforeSymbol, afterString, beforeString, afterContent, beforeContent, functionHint, functionEnd, occurrence = 1, dryRun = false, lint = false }) => {
        console.log(`CMD: ${curTool}, ARGS:`, { insert_line, new_str: new_str.substring(0, 80), endOfFile, afterFunction, beforeFunction, afterSymbol, beforeSymbol, afterString, beforeString, afterContent, beforeContent, functionHint, functionEnd, occurrence, dryRun });
        ...
        if (endOfFile) {
          ...
          insertFailures.count = 0; bump('insert', 'hits');
        ...
        if (afterFunction !== undefined || beforeFunction !== undefined) {
          ...
          const allSymbols = getSymbols(editor, null, editor.getPath());
          const r = resolveSymbolPosition(allSymbols, hintVal, 'function', dir);
          if (r.notFound) {
            insertFailures.count++;
            bump('insert', 'fails.anchorNotFound');
            const known = allSymbols.filter(s => ['function','registerTool'].includes(s.kind)).map(s => `  ${s.startRow + 1}: ${s.name}`).slice(0, 20);
            return { content: [{ type: 'text', text: [`❌ ${hintName}: function "${hintVal}" not found.`, `Known functions:\n${known.join('\n') || '  (none)'}`, smartSuggestion({ toolName: curTool, counter: insertFailures, noHintsUsed: false, fileLines: allLines.length, oldStr: null, isCodeFile: isCodeFilePath(editor.getPath()) })].filter(Boolean).join('\n') }], inserted: false };
          }
          if (r.ambiguous) {
            insertFailures.count++;
            bump('insert', 'fails.anchorNotFound');
          ...
          if (dryRun) {
            ...
            const ce = Math.min(allLines.length - 1, insertRow + rd);
            const ctxLines = allLines.slice(cs, insertRow).map((l, i) => `${String(cs + i + 1).padStart(4)}   ${l}`);
            const insLines = new_str.split('\n').map(l => `${' '.repeat(4)} ► ${l}`);
          ...
          const _styleSuffixAF = applyStyleCheck(new_str, editor.getPath());
        ...
        if (afterSymbol !== undefined || beforeSymbol !== undefined) {
          ...
          const _styleSuffixAS = applyStyleCheck(new_str, editor.getPath());
        ...
        if (afterString !== undefined || beforeString !== undefined) {
          ...
          if (r.notFound) {
            insertFailures.count++;
            bump('insert', 'fails.anchorNotFound');
            return { content: [{ type: 'text', text: [`❌ ${hintName}: "${hintVal.substring(0, 60)}" not found in file.`, smartSuggestion({ toolName: curTool, counter: insertFailures, noHintsUsed: false, fileLines: allLines.length, oldStr: null, isCodeFile: isCodeFilePath(editor.getPath()) })].filter(Boolean).join('\n') }], inserted: false };
          }
          if (r.ambiguous) {
            insertFailures.count++;
            bump('insert', 'fails.anchorNotFound');
          ...
          const _styleSuffixSS = applyStyleCheck(new_str, editor.getPath());
        ...
        if (functionEnd !== undefined) {
          ...
          return {
            ...buildEditResponse(
              { tool: curTool, line: insertRow + 1, linesChanged: new_str.split('\n').length, scopeLabel, tags: ['staleLinesWarning'] },
              { lint: _lintSuffix, style: _styleSuffix }
            ),
            dryRun: false, newLineCount
          };
        ...
        if (afterContent !== undefined || beforeContent !== undefined) {
          ...
          if (!hit) {
            ...
            bump('insert', 'fails.anchorNotFound');
            logFailure({
              tool: curTool, reason: 'anchorNotFound', filePath: editor.getPath(),
              lineNumberHint: null,
              hintsSet: [functionHint && 'functionHint', afterContent !== undefined && 'afterContent', beforeContent !== undefined && 'beforeContent'].filter(Boolean),
              oldStrPreview: anchor, diffVsBuffer: null,
              bufferPreview: (() => {
                const _fn = functionHint ? findFunctionInBuffer(buffer, functionHint) : null;
          ...
          return {
            ...buildEditResponse(
              { tool: curTool, line: anchorRow + 1, linesChanged: new_str.split('\n').length, scopeLabel, tags: ['staleLinesWarning'] },
              { lint: _lintSuffix2, style: _styleSuffix2 }
            ),
            dryRun: false, newLineCount
          };
        }
        ...
        insertFailures.count = 0; bump('insert', 'hits'); // reset on success
        ...
        return {
          ...buildEditResponse(
            { tool: curTool, line: insert_line, linesChanged: new_str.split('\n').length, tags: ['staleLinesWarning'] },
    ...
    server.registerTool(
      curTool,
      {
        ...
        inputSchema: {
          ...
          ...ANCHOR_SCHEMA,  // functionHint, afterHint, betweenHint, lineNumberHint, occurrence, dryRun
        }
      },
      async ({ startLine, endLine, dryRun = false, functionHint, afterHint, lineNumberHint, betweenHint, occurrence = 1, fuzzyWhitespace, lint = false }) => {
        console.log(`CMD: ${curTool}, ARGS:`, { startLine, endLine, dryRun, functionHint, afterHint, lineNumberHint, betweenHint, occurrence, fuzzyWhitespace });
        ...
        // afterHint / lineNumberHint: sets startLine to the resolved row + 1 (endLine unchanged).
        if (functionHint) {
          ...
          const text = buffer.getText();
          const syms = getSymbols(editor, text, editor.getPath());
          ...
          const text = buffer.getText();
          const syms = getSymbols(editor, text, editor.getPath());
          const r = resolveAnchor(afterHint, syms, text);
          const err = anchorError('afterHint', afterHint, r);
          if (err) { bump('delete_line_range', 'fails.outOfRange'); return { content: [{ type: "text", text: err }], deleted: false }; }
          startLine = r.row + 2; // start on the line AFTER the anchor
        } else if (lineNumberHint) {
          bump('delete_line_range', 'hintsUsed.lineNumberHint');
          startLine = lineNumberHint;
        }
        ...
        if (startLine < 1 || endLine < 1) {
          ...
          return { content: [{ type: "text", text: [
            ...
            smartSuggestion({ toolName: "delete-line-range", counter: deleteFailures, noHintsUsed: !functionHint && !afterHint && !betweenHint && !lineNumberHint, fileLines: lineCount, oldStr: null, isCodeFile: isCodeFilePath(editor.getPath()) })
        ...
        deleteFailures.count = 0; bump('delete_line_range', 'hits'); // reset on success
  ...
  // ── P6: delete-block ─────────────────────────────────────────────────────
  {
    ...
    server.registerTool(
      ...
      async ({ startContent, endContent, sectionHint, preprocBlock, preprocSide, inclusive = true, functionHint, afterHint, betweenHint, lineNumberHint, occurrence = 1, dryRun = false, lint = false }) => {
        ...
        if (sectionHint !== undefined || preprocBlock !== undefined) {
          ...
          deleteBlockFailures.count = 0; bump('delete_block', 'hits');
          const newLineCount = buffer.getLineCount();
          const _lintSuffix = await maybeLintSuffix(lint, editor, delStart, delStart + 5);
          return {
            ...buildEditResponse(
              { tool: curTool, line: delStart + 1, linesChanged: -deletedCount, tags: ['staleLinesWarning'] },
              { lint: _lintSuffix }
            ),
            dryRun: false, deleted: true, newLineCount, deletedCount, staleLinesWarning: true
          };
        ...
        if (startContent) {
          ...
          const _noScope = !functionHint && !afterHint && !betweenHint && !lineNumberHint && occurrence <= 1;
  ...
  // ── P7: replace-block ─────────────────────────────────────────────────────
  {
    ...
    server.registerTool(
      ...
      async ({ anchor, newBody, occurrence = 1, dryRun = false, functionHint, afterHint, lineNumberHint, betweenHint, fuzzyWhitespace, lint = false }) => {
        console.log(`CMD: ${curTool}, ARGS:`, { anchor: anchor.substring(0, 80), newBodyLength: newBody.length, occurrence, dryRun, functionHint, afterHint, lineNumberHint, betweenHint, fuzzyWhitespace });
        ...
        // functionHint > betweenHint > afterHint > lineNumberHint > entire file
        ...
        if (functionHint) {
          ...
          const r = resolveAnchor(afterHint, syms, text);
          const err = anchorError('afterHint', afterHint, r);
          if (err) { bump('replace_block', 'fails.anchorNotFound'); return { content: [{ type: "text", text: err }], found: false }; }
          searchStartRow = r.row + 1; // start searching from the row AFTER the anchor
        } else if (lineNumberHint) {
          bump('replace_block', 'hintsUsed.lineNumberHint');
          searchStartRow = Math.max(0, lineNumberHint - 1);
        }
        ...
        if (anchorRow === -1) {
          bump('replace_block', 'fails.anchorNotFound');
          replaceBlockFailures.count++;
          logFailure({
            tool: curTool,
            reason: 'anchorNotFound',
            filePath: editor.getPath(),
            lineNumberHint: lineNumberHint || null,
            hintsSet: [functionHint && 'functionHint', afterHint && 'afterHint', betweenHint && 'betweenHint', lineNumberHint && 'lineNumberHint'].filter(Boolean),
          ...
          return { content: [{ type: 'text', text: [
            ...
            smartSuggestion({ toolName: curTool, counter: replaceBlockFailures, noHintsUsed: !functionHint && !afterHint && !betweenHint && !lineNumberHint, fileLines: allLines.length, oldStr: anchor, isCodeFile: isCodeFilePath(editor.getPath()) })
          ].filter(Boolean).join('\n') }], found: false };
        }
        ...
        replaceBlockFailures.count = 0; bump('replace_block', 'hits');
        return {
          ...buildEditResponse(
    ...
    server.registerTool(
      curTool,
      {
        ...
        description: [
          ...
          "After reading with get-region, use the returned line numbers directly with str_replace lineNumberHint, or use betweenHint with the same anchor strings to scope an edit."
        ].join(" "),
        inputSchema: {
    ...
    server.registerTool(curTool,
      ...
      async ({ command, cwd, timeout = 30000, confirm = false, showOutput = true }) => {
        ...
        const preFlightLines = [
          `ℹ️ run-command pre-flight: checkpoint '${cpName}' saved for ${openEditors.length} open file(s).`
        ];
          ...
          proc.on('close', (code) => {
            ...
            if (reloaded.length > 0 && chatPanel && showOutput) {
              chatPanel.appendOutput(
                `ℹ️ Reloaded ${reloaded.length} file(s) changed by command:\n` +
    ...
    server.registerTool(curTool,
      ...
      async ({ query, replacement, regex = false, caseSensitive = false, glob = "",
        ...
        if (committing) {
          ...
          for (const { filePath } of rafPending.files) {
            ...
            const updated = original.replace(_raf_re, function(m) {
              ...
              return applyReplacement(replacement, m, groups, pre, post);
            });
            if (count === 0) continue;
        ...
        if (capped) {
          return {
            content: [{ type: "text", text: JSON.stringify({
              summary: `⚠  Match cap hit (${maxMatches}). Too many matches to safely preview —" narrow the scope with glob or a more specific query, or raise maxMatches if you're sure.`,
    ...
    server.registerTool(curTool,
      {
        ...
        description: [
          ...
          "DISAMBIGUATION: if the same function name appears in multiple places (e.g. declaration + definition), use afterHint or lineNumberHint to identify which one to target. occurrence:N also works.",
      ...
      async ({ name, newBody, dryRun = false, occurrence = 1, functionHint, afterHint, lineNumberHint, betweenHint, fuzzyWhitespace, lint = false }) => {
        console.log(`CMD: ${curTool}, ARGS:`, { name, newBodyLength: newBody.length, dryRun, occurrence, functionHint, afterHint, lineNumberHint, betweenHint, fuzzyWhitespace });

        const editor = atom.workspace.getActiveTextEditor();
        if (!editor) throw new Error("No active editor");

        // Derive function name from first line of newBody (e.g. "int hal_init(..." -> "hal_init")
        // Falls back to functionHint if provided. name is used for findFunction + error messages.
        const _sigLine = newBody.trimStart().split(/\r?\n/)[0] ?? '';
        const _nameMatch = _sigLine.match(/\b([A-Za-z_][A-Za-z0-9_]*)\s*\(/);
        const _fnName = (_nameMatch && _nameMatch[1]) || functionHint || null;
        ...
        if (functionHint) bump('replace_function_body', 'hintsUsed.functionHint');
        if (betweenHint)  bump('replace_function_body', 'hintsUsed.betweenHint');
        if (afterHint)    bump('replace_function_body', 'hintsUsed.afterHint');
        if (lineNumberHint)     bump('replace_function_body', 'hintsUsed.lineNumberHint');
        ...
        // Translate hints to row offsets for findFunction via resolveAnchor
        // allSymbols is computed below — hoist a quick parse here for hint resolution
        const _hintSyms = getSymbols(editor, text, editor.getPath());
        let afterRow    = undefined;
        let betweenRows = undefined;
        if (afterHint) {
          const r = resolveAnchor(afterHint, _hintSyms, text);
          const err = anchorError('afterHint', afterHint, r);
          if (err) { bump('replace_function_body', 'fails.ambiguousHint'); return { content: [{ type: "text", text: err }], replaced: false }; }
          afterRow = r.row;
        }
        if (betweenHint) {
        ...
        // lineNumberHint: convert 1-based to 0-based for proximity search
        const lineNumberHintRow = lineNumberHint != null ? Math.max(0, lineNumberHint - 1) : undefined;

        // ── Symbol extraction — tree-sitter first, regex fallback ────────────
        const allSymbols = getSymbols(editor, text, editor.getPath());
        console.log(`[tss] allSymbols: ${allSymbols.length}, kinds: ${[...new Set(allSymbols.map(s=>s.kind))].join(',')}`);

        // If functionHint is given, scope to that outer function's row range
        let scopedSymbols = allSymbols;
        if (functionHint) {
          const outer = findFunction(allSymbols, functionHint, {});
          if (outer) {
        ...
        const found = findFunction(scopedSymbols, _fnName, {
          occurrence,
          lineNumberHint:    lineNumberHintRow,
          afterRow,
          betweenRows,
        });
        ...
        if (!found) {
          ...
          logFailure({
            tool: curTool, reason: 'notFound', filePath: editor.getPath(),
            lineNumberHint: lineNumberHint || null,
            hintsSet: [afterHint && 'afterHint', betweenHint && 'betweenHint', lineNumberHint && 'lineNumberHint', functionHint && 'functionHint'].filter(Boolean),
            oldStrPreview: _fnName, diffVsBuffer: null,
            availableFunctions: allNames.slice(0, 20),
            closestMatches: close.map(x => x.fn),
            bufferPreview: (() => {
              // Show first 15 lines of search scope so caller can see what's actually there
              const _s = lineNumberHintRow != null ? Math.max(0, lineNumberHintRow - 3) : 0;
              return allLines.slice(_s, Math.min(allLines.length - 1, _s + 15) + 1)
                .map((l, i) => `L${_s + i + 1}: ${l}`).join('\n');
            })()
          });
          ...
          if (_noScope) {
            ...
            if (dupes.length > 1) {
              ...
              return { content: [{ type: "text", text: [
                ...
                `   Use occurrence:N, lineNumberHint, afterHint, or betweenHint to target one:`,
                `   • occurrence:N   → Nth definition (1=line ${dupes[0].startRow + 1}, 2=line ${dupes[1]?.startRow + 1 ?? "?"}, ...)`,
                `   • lineNumberHint:N     → picks definition closest to line N`,
                `   • afterHint:"…"  → start search after a unique anchor line`,
        ...
        return {
          ...buildEditResponse(
            { tool: curTool, line: startRow + 1, linesChanged: insertedLines - (endRow - startRow + 1), scopeLabel: ' "' + _fnName + '"',
              tags: signatureChanged ? ['signatureChanged'] : [] },
            { lint: _lintSuffix, style: _styleSuffix }
          ),
...

  {
    ...
    server.registerTool(curTool,
      ...
      async ({ filePath } = {}) => {
        ...
        // ── Function extraction via tree-sitter (regex fallback inside getSymbols) ──
    ...
    server.registerTool(curTool,
      {
        ...
        description: [
          ...
          "occurrence:N — return only the Nth match with its context. Use this when you know which specific instance you want (e.g. the 3rd call to a function) — tells you the exact line number to pass to str_replace lineNumberHint or read-lines.",
      ...
      async ({ query, filePath, regex = false, caseSensitive = false, maxMatches = 200, contextLines = 0, occurrence = 0 }) => {
        ...
        if (matches.length === 0) {
          bump('grep_file', 'fails.noMatch');
          return { content: [{ type: 'text', text: JSON.stringify({ matchCount: 0, truncated: false, matches: [] }) }] };
        }
        bump('grep_file', 'hits');
    ...
    server.registerTool(curTool,
      {
        ...
        description: [
          ...
          "(4) Have a line number and want context around it? → lineNumberHint:42 (defaults to radius:10, i.e. 10 lines above and below) or centerLine:42 with radius:N.",
          ...
          "Returns lines with their original 1-based line numbers — use them directly as lineNumberHint in str_replace or startLine/endLine in delete-line-range."
        ].join(" "),
        inputSchema: {
          startLine:    z.number().optional(),
          endLine:      z.number().optional(),
          ...
          lineNumberHint:     z.number().optional(),
        }
      },
      async ({ startLine, endLine, filePath, centerLine, radius, functionHint, afterHint, betweenHint, lineNumberHint }) => {
        console.log(`CMD: ${curTool}, ARGS:`, { startLine, endLine, filePath, centerLine, radius, functionHint, afterHint, betweenHint, lineNumberHint });
        ...
        // ── Resolve range from hints via tree-sitter ─────────────────────────
        const _editor  = filePath ? null : atom.workspace.getActiveTextEditor();
        const _syms    = getSymbols(_editor, text, resolvedPath);

        if (functionHint) {
          const sym = _syms.find(s => s.name === functionHint);
          if (!sym) throw new Error(`functionHint: function "${functionHint}" not found in ${resolvedPath}. Known: ${_syms.map(s=>s.name).slice(0,10).join(', ')}`);
          ...
          const r = resolveAnchor(afterHint, _syms, text);
          const err = anchorError('afterHint', afterHint, r);
          ...
          _hintLabel    = ` [afterHint [${r.via}] "${afterHint.substring(0, 40)}" line ${resolvedStart}]`;

        } else if (centerLine !== undefined || lineNumberHint !== undefined) {
          const center = centerLine !== undefined ? centerLine : lineNumberHint;
          ...
          if (resolvedStart === undefined || resolvedEnd === undefined)
            throw new Error("Must provide startLine+endLine, centerLine, functionHint, afterHint, betweenHint, or lineNumberHint.");
        }
        ...
        else if (lineNumberHint !== undefined)               bump('read_lines', 'hintsUsed.lineNumberHint');
        else if (centerLine !== undefined)             bump('read_lines', 'hintsUsed.lineNumberHint'); // centerLine maps to same bucket
        return {
  ...
  if (g('edit')) {
  {
    ...
    server.registerTool(curTool,
      ...
      async ({ query, replacement, regex = false, caseSensitive = false, dryRun = false, lint = false }) => {
        ...
        // Count matches via a separate pass (test() on a non-global clone per line).
        ...
        return {
          ...buildEditResponse(
            { tool: curTool, scopeLabel: ` — ${matchCount} occurrence${matchCount === 1 ? '' : 's'} of ${JSON.stringify(query)}` },
            { lint: _lintSuffix, style: _styleSuffix }
          ),
    ...
    server.registerTool(
      ...
      async ({ filePath } = {}) => {
        ...
        if (filePath) {
          ...
          try { _gsText = await readFileOrBuffer(filePath); }
  ...
  // no subprocess, no disk write required, full undo/redo and decoration support.
  {
    ...
    server.registerTool(curTool,
      ...
      async ({ expression, functionHint, dryRun = false, lint = false }) => {
          ...
          for (let i = scopeStart; i <= scopeEnd; i++) {
            ...
            if (nthOnly) {
              ...
              const newLine = line.replace(subRe, function(m) {
                ...
                const groups = Array.prototype.slice.call(arguments, 1, arguments.length - 2);
                const offset = arguments[arguments.length - 2];
                return applyReplacement(rep, m, groups, line.slice(0, offset), line.slice(offset + m.length));
              });
        ...
        buffer.setTextViaDiff(newText);
        decorateEditedLines(editor, originalText, newText);
        sedFailures.count = 0; bump('sed', 'hits');
  ...
  if (g('fileOps')) {
  {
    ...
    server.registerTool(curTool,
      ...
      async ({ patch = "", dryRun = false, fuzzFactor = 0, confirm = false, lint = false }) => {
        ...
        patchFailures.count = 0; bump('apply_patch', 'hits');
    ...
    server.registerTool(curTool,
      ...
      async ({ glob = "", excludeGlob = "", maxTokens = 1024, minRefs = 0, mentionedFiles = [], includeLineNumbers = true } = {}) => {
        ...
        for (const fp of allFiles) {
          ...
          const checkMeCount = (txt.match(/\/\*\s*CHECK:/g) || []).length;
...

  {
    ...
    server.registerTool(curTool, {
      ...
      const symbols = getSymbols(editor, editor.getText(), editor.getPath() || '');
...

  {
    ...
    server.registerTool(curTool, {
      ...
      const symbols = getSymbols(editor, text, editor.getPath() || '');
  ...
  {
    const curTool = "get-failure-log";
    server.registerTool(curTool,
      {
        title: "Get Failure Log",
        description: [
          "Query the persistent failure log (failure-log.ndjson) — a structured record of every tool failure written to disk.",
          "Each entry includes: ts (ISO timestamp), tool, reason, filePath, lineNumberHint, and other context.",
      ...
      async ({ tail = 20, tool, reason, filePath: fpFilter }) => {
        dbg(curTool, "ARGS", { tail, tool, reason, fpFilter });
        ...
        try {
          ...
          return { content: [{ type: "text", text: "(failure-log.ndjson not found — no failures recorded yet)" }] };
        }
  ...
  // -------------------------------------------------------------------------
  {
    ...
    async ({ filePath }) => {
      const curStats = editStats[curTool] || (editStats[curTool] = { hits: 0, fails: { notKernel: 0 }, misses: { clean: 0 } });
      const editor = atom.workspace.getActiveTextEditor();
      ...
      if (!isKernelFile(targetPath)) {
        ...
        return { content: [{ type: "text", text: `⚠️ namingcheck: not a kernel C file (${path.basename(targetPath)}) — skipped.` }] };
      }
      let text;
      try { text = await readFileOrBuffer(targetPath); }
  ...
  // -------------------------------------------------------------------------
  {
    const curTool = "check-function-docs";
    server.registerTool(curTool, {
    ...
    async ({ filePath }) => {
      const curStats = editStats[curTool] || (editStats[curTool] = { hits: 0, fails: { notKernel: 0 }, misses: { allGood: 0 } });
      const editor = atom.workspace.getActiveTextEditor();
      const targetPath = filePath || editor?.getPath();
      if (!targetPath) return { content: [{ type: "text", text: "❌ No file path provided and no active editor." }] };
      if (!isKernelFile(targetPath)) {
        curStats.fails.notKernel++;
        return { content: [{ type: "text", text: `⚠️ check-function-docs: not a kernel C file (${path.basename(targetPath)}) — skipped.` }] };
      }
      let text;
      try { text = await readFileOrBuffer(targetPath); }
      catch (err) { return { content: [{ type: "text", text: `❌ Cannot read file: ${err.message}` }] }; }
  ...
  // -------------------------------------------------------------------------
  {
    ...
    server.registerTool(curTool, {
      ...
      description: [
        ...
        "filePath: path to the file (defaults to active editor if omitted).",
      ].join(" "),
      inputSchema: {
    ...
    async ({ functionName, filePath, line: hintLine }) => {
      const curStats = editStats[curTool] || (editStats[curTool] = { hits: 0, fails: { notKernel: 0, notFound: 0, noEditor: 0 } });
      const editor = filePath
        ? atom.workspace.getTextEditors().find(e => e.getPath() === filePath) || await atom.workspace.open(filePath, { activatePane: false })
      ...
      const _docMatch = findFunction(_docSyms, functionName, hintLine !== undefined ? { lineNumberHint: hintLine } : {});
      if (_docMatch) {
...
// (patchFailures, strReplFailures, insertFailures, deleteFailures, replaceBlockFailures declared at module scope above)
// ---------------------------------------------------------------------------
// Decoration helpers
// ---------------------------------------------------------------------------
