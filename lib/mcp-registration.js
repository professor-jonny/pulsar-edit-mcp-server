...
const { resolveScope, matchContent: matchContentEngine, matchBlock, matchFunction, applyEdit, buildFailResponse, diagnoseFailure, buildInsertPreview, buildReplacePreview, retryNudge } = require('./match-engine'); // buildReplacePreview added 2026-09-25 for the new 'replace' tool (decision A: parallel engine-native sibling to str_replace, for comparison)
...
function mcpRegistration(server, linterRegistry = null, getMessages = null, groups = {}, chatPanel = null, getCommandTerminalPanel = null) {
      ...
      const _hasScope = !!(inFunction || betweenHint ||
        ...
        if (matchIndex === -1) {
          ...
          if (words.length > 0) {
            ...
            // Plausibility floor: on a needle with only 1-2 meaningful words, a single word
            // match is often the whole signal available (e.g. a one-word typo still shares
            // its other word) and is kept. From 3 meaningful words up, require at least half
            // to have matched before trusting fuzzyRow -- otherwise a single incidental word
            // match on a long/repetitive-vocabulary file can win `bestScore` and silently
            // point "Closest area found" at the wrong row.
          ...
          //    The "Closest area found" CONTEXT BLOCK is only shown when fuzzyRow cleared
          //    the plausibility floor above -- that block asserts a specific location, which
          //    must not be shown on a weak/implausible fuzzyRow. Similarity still computes
          //    (falling back to diagOffset when fuzzyRow is -1); the "Low match" tier already
          //    communicates that the location is uncertain, so it's not a location claim.
          if (fuzzyRow >= 0) {
  ...
  // ── replace (2026-09-25, decision A) ────────────────────────────────────────
  // Parallel sibling to str_replace, built 100% on the shared match engine from
  // day one (resolveScope/matchContent/applyEdit/buildFailResponse/buildReplacePreview
  // -- the same primitives insert/delete/replace-block already use) instead of
  // str_replace's own hand-rolled matching + rescue code. Purpose: a clean
  // baseline to compare against str_replace on the same edits. Own stats
  // namespace ('replace', not 'str_replace') so traffic never mixes -- see
  // mcp-server-refactor-plan.md's "OPEN DECISIONS FOR PJ" section, decision A.
  // Decision B (same session): NO sub-line/fragment matching here -- old_str
  // must match one or more WHOLE lines. That keeps the S3 ruling (2026-09-20,
  // part-line search/replace rejected as a NEW feature) intact; fragment
  // matching stays inside str_replace only.
  if (g('edit')) registerMcpTool({
    name:     'replace',
    group:    'edit',
    category: 'edit',
    features: { dryRun: true, lint: true, consecutiveFailureCounter: true },
    title:    'Replace (engine-native)',
    description: [
      "Replace one or more COMPLETE lines matching `old_str` with `new_str` in the file at `filePath` (required).",
      "EXPERIMENTAL / FOR COMPARISON: a from-scratch sibling of `str_replace` built entirely on the shared match engine (same resolveScope/matchContent/applyEdit pipeline every other migrated tool uses) instead of str_replace's own hand-rolled matching and rescue code. Use it to compare behavior against str_replace on the same edit; str_replace remains the reference tool for everyday use.",
      "old_str must match one or more WHOLE lines exactly (after any fuzzy tolerance requested) -- it cannot match a fragment in the middle of a line, unlike str_replace. For a mid-line substring edit, use str_replace instead.",
      "occurrence:N -- when old_str matches more than once in scope, targets the Nth occurrence; an ambiguous match with no occurrence is refused, never guessed.",
      "fuzzyWhitespace:true / fuzzyContent:true -- opt-in tolerance for indentation drift / unicode punctuation differences.",
      "regex:true -- treat old_str as a regular expression ('gm' flags -- ^/$ are line anchors, not whole-text anchors).",
      "dryRun:true -- previews the match and diff without writing. Always use this first for anything you're not certain about.",
      "Hint params (inFunction, afterString, beforeString, afterFunction, beforeFunction, afterSymbol, beforeSymbol, afterLine, beforeLine, betweenHint, hintRadius) scope the search exactly as they do for str_replace/insert/delete/replace-block."
    ].join(" "),
    inputSchema: {
      old_str:         z.string(),
      new_str:         z.string().optional(),
    ...
    handler: async ({ old_str, new_str = '', occurrence, dryRun = false, inFunction, afterFunction, beforeFunction, afterSymbol, beforeSymbol, afterString, beforeString, afterLine, beforeLine, betweenHint, hintRadius, fuzzyWhitespace, fuzzyContent, regex, lint = false }, ctx) => {
      const { editor, buffer, allLines } = ctx;
      const curTool = 'replace';
      const isCodeFile = isCodeFilePath(ctx.filePath);

      if (inFunction)              bump(curTool, 'hintsUsed.inFunction');
      if (betweenHint)             bump(curTool, 'hintsUsed.betweenHint');
      if (afterString)             bump(curTool, 'hintsUsed.afterString');
      if (beforeString)            bump(curTool, 'hintsUsed.beforeString');
      if (afterFunction)           bump(curTool, 'hintsUsed.afterFunction');
      if (beforeFunction)          bump(curTool, 'hintsUsed.beforeFunction');
      if (afterSymbol)             bump(curTool, 'hintsUsed.afterSymbol');
      if (beforeSymbol)            bump(curTool, 'hintsUsed.beforeSymbol');
      if (afterLine != null)       bump(curTool, 'hintsUsed.afterLine');
      if (beforeLine != null)      bump(curTool, 'hintsUsed.beforeLine');
      if (occurrence != null)      bump(curTool, 'hintsUsed.occurrence');
      if (fuzzyWhitespace)         bump(curTool, 'hintsUsed.fuzzyWhitespace');
      ...
      if (scope.error) {
        if (scope.error.kind !== 'ambiguous') ctx.consec.count++;
        bump(curTool, (scope.error.kind === 'ambiguous') ? 'fails.anchorAmbiguous' : 'fails.anchorNotFound');
        return { ...buildFailResponse({ tool: curTool, ctx: { consec: ctx.consec, allLines, text: ctx.text }, scope, needle: old_str, isCodeFile }), found: false };
      }

      const match = matchContentEngine(old_str, { allLines, text: ctx.text }, scope, { fuzzyWhitespace, fuzzyContent, regex, occurrence });

      if (!match.matched) {
        if (match.reason !== 'ambiguous') ctx.consec.count++;
        return { ...buildFailResponse({ tool: curTool, ctx: { consec: ctx.consec, allLines, text: ctx.text }, scope, match, needle: old_str, isCodeFile }), found: false };
      }

      const _tags = (match.transforms || []).slice();
      if ((match.transforms || []).includes('fuzzyWhitespace')) bump(curTool, 'fuzzyTriggerReasons.needsWhitespace');
      if ((match.transforms || []).includes('fuzzyContent'))    bump(curTool, 'fuzzyTriggerReasons.needsContent');
      if ((match.transforms || []).includes('partial'))         bump(curTool, 'fuzzyTriggerReasons.partial');

      if (dryRun) {
        bump(curTool, 'dryRuns');
        return buildReplacePreview({
          allLines, lineCount: allLines.length,
          matchLine: match.matchLine, matchLines: match.actualLines,
          new_str, scopeLabel: scope.scopeLabel || '', tags: _tags,
        });
      }

      const originalText = buffer.getText();
      const edit = applyEdit(buffer, match, new_str);
      if (edit.error) {
        bump(curTool, 'fails.noMatch');
        return { content: [{ type: 'text', text: `❌ ${curTool}: ${edit.error} — the buffer changed since the match was resolved; re-run to re-resolve.` }], found: false };
      }
      decorateEditedLines(editor, originalText, buffer.getText());
      ctx.consec.count = 0; bump(curTool, 'hits');
      if ((match.transforms || []).includes('fuzzyWhitespace')) bump(curTool, 'fuzzyWhitespaceCommits');
      if ((match.transforms || []).includes('fuzzyContent'))    bump(curTool, 'fuzzyContentCommits');
      if (regex) bump(curTool, 'regexCommits');

      const newLineCount = buffer.getLineCount();
      const linesChanged = new_str.split("\n").length - (match.matchEndLine - match.matchLine + 1);
      const _lintSuffix  = await maybeLintSuffix(lint, editor, match.matchLine, match.matchLine + new_str.split("\n").length - 1);
      const _styleSuffix = applyStyleCheck(new_str, ctx.filePath);

      return {
        ...(await buildEditResponse(
          { tool: curTool, line: match.matchLine + 1, linesChanged, scopeLabel: scope.scopeLabel || '', tags: _tags, buffer },
          { lint: _lintSuffix, style: _styleSuffix }
        )),
        found: true, dryRun: false, matchLine: match.matchLine + 1, matchEndLine: match.matchEndLine + 1, newLineCount
      };
    },
  });
