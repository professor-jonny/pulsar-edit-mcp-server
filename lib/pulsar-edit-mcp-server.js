...
const CommandTerminalPanel = require('./command-terminal-panel');
...
function loadMcpModules() {
  ...
  try {
    ...
    // tool-framework.js, schema.js, and lint-helpers.js are dependencies of
    // mcp-registration.js and must also be invalidated, otherwise changes to
...
let commandTerminalPanelRef = null;   // set by activate(), read by run-command's spawn handler
...
app.post('/mcp', async (req, res) => {
  ...
  if (sessionId && transports[sessionId]) {
    ...
    await initStats();
    mcpRegistration(mcpServer, linterRegistry, () => latestLinterMessages, groups, chatPanelRef, () => commandTerminalPanelRef); // Register core editor tools
...
module.exports = {
  ...
  async activate(state) {
    ...
    */
    ...
    this.subscriptions.add(
      atom.commands.add('atom-workspace', {
      'pulsar-edit-mcp-server:show-command-terminal-panel': () => {
        atom.workspace.open('atom://pulsar-edit-mcp-server/command-terminal');
      }
    }));

    this.subscriptions.add(
      atom.workspace.addOpener(uri => {
        if (uri === 'atom://pulsar-edit-mcp-server/command-terminal') {
          this.commandTerminalPanel = new CommandTerminalPanel();
          commandTerminalPanelRef = this.commandTerminalPanel;
          return this.commandTerminalPanel;
        }
      })
    );
    ...
    // Command Output panel is never in the user's saved layout by default and,
    // unlike the chat panel, nothing else ever requests its URI — so unless we
    // open it once here, commandTerminalPanelRef stays null forever and
    // run-command's own auto-open (which requires a non-null panel) can never
    // fire. Open it in the background so it exists and is ready to be
    // shown/focused the first time a command actually needs it.
    await atom.workspace.open('atom://pulsar-edit-mcp-server/command-terminal', { activatePane: false, activateItem: false });
    ...
    if (!commandTerminalPanelRef) {
      const existingTerminal = atom.workspace.getPaneItems().find(i => i instanceof CommandTerminalPanel);
      if (existingTerminal) commandTerminalPanelRef = existingTerminal;
    }
          ...
          const GROUP_DEFS = [
            ...
            },
            {
              ...
              keys: ['grep_file','grep_project','search_symbol','find_text',
                     ...
                     'read','get_selection','get_linter_messages'],
            },
            {
              groupKey: null,
