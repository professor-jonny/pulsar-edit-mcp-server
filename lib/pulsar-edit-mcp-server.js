...
function loadMcpModules() {
  ...
  try {
    // Delete cached module so hot-reload gets a fresh copy
    const regPath = require.resolve('./mcp-registration.js');
...
module.exports = {
  ...
  listenToggle() {
    ...
    if (this.listening) {
      ...
      this.serverInstance.once('listening', () => {
        startMcpClient(this.chatPanel || chatPanelRef).then(client => { this.mcpClient = client; });
      });
    }
            ...
            for (const key of groupTools) {
              ...
              if (rows.length) {
                ...
                body.innerHTML = rows.map(r =>
                  `<div style="opacity:0.85">${r.replace(/&/g,'&amp;').replace(/</g,'&lt;')}</div>`
                ).join('');
  ...
  showFaultLog() {
    ...
    const LOG_PATH = require('path').join(PKG_PATH, 'failure-log.ndjson');
    ...
    function renderTable() {
      ...
      rows.forEach((e, idx) => {
        ...
        const line = e.lineNumberHint != null ? String(e.lineNumberHint) : '–';
        const skip = new Set(['ts','tool','reason','filePath','lineNumberHint']);
    ...
    function showEntryDetail(e, rowNum) {
      ...
      // ── field grid ───────────────────────────────────────────────────
      const FIELD_ORDER = ['ts', 'tool', 'reason', 'filePath', 'lineNumberHint'];
    ...
    clearBtn.addEventListener('click', () => {
      if (!confirm('Delete all entries in failure-log.ndjson? This cannot be undone.')) return;
