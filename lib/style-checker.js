 ...
 * @param {boolean} [isWholeFile] — true for whole-file rewrites (enables EOF newline check)
 * @param {number} [lineOffset] — 0-based row where newStr starts in the buffer;
 *   added to each violation's line number so it reads as an absolute buffer line
 *   instead of a line relative to the start of newStr. Default 0 preserves the
 *   original snippet-relative behavior for callers that don't know the absolute
 *   position (str_replace, replace_block, replace_function_body, apply_patch).
 * @returns {{ violations: Array, totalViolations: number }}
 */
function checkLines(newStr, filePath, isWholeFile, lineOffset = 0) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const lineNum = lineOffset + i + 1;  // 1-based, shifted to absolute buffer line when lineOffset is set
    const raw = lines[i];
 ...
 * Caps the rendered list at MAX_RENDERED violations to avoid burning tokens
 * on large multi-line inserts with pervasive style issues — the full count
 * is still shown, and a summary line replaces the remainder rather than
 * listing each one, since past the first handful the LLM already has
 * enough signal to know the edit needs a review pass.
 */
const MAX_RENDERED_VIOLATIONS = 8;

function formatViolations(violations) {
  if (!violations || violations.length === 0) return null;
  const shown = violations.slice(0, MAX_RENDERED_VIOLATIONS);
  const parts = shown.map(v => `[L${v.line}] ${v.type.replace(/_/g, ' ')}`);
  const remaining = violations.length - shown.length;
  if (remaining > 0) {
    parts.push(`+${remaining} more — this edit has widespread style issues, review it directly rather than relying on this list`);
  }
