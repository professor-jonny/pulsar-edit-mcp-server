...
function resolveStructuralAnchor(buffer, { sectionHint, preprocBlock, preprocSide, occurrence } = {}) {
  ...
  if (sectionHint) {
    ...
    // S1: an occurrence past the last match is REFUSED, never clamped -- clamping made
    // delete{sectionHint, occurrence:5} silently target the last banner.
    if (occurrence != null && (!Number.isInteger(occurrence) || occurrence < 1 || occurrence > matches.length)) {
      return { ambiguous: true, outOfRange: true, occurrence, matches };
    }
  ...
  if (preprocBlock) {
    ...
    // S1: same refuse-don't-clamp rule as sectionHint above.
    if (occurrence != null && (!Number.isInteger(occurrence) || occurrence < 1 || occurrence > matches.length)) {
      return { ambiguous: true, outOfRange: true, occurrence, matches: matches.map(m => ({ startRow: m.openRow, endRow: m.closeRow })) };
    }
    const { openRow, closeRow } = matches[(occurrence ?? 1) - 1];
...
// read-file, get-repo-map, read, search-symbol,
