...
// hintName:  'afterString' | 'beforeString' | 'afterLine' | 'beforeLine' | 'betweenHint.start' | 'betweenHint.end'
...
function anchorError(hintName, hintValue, result, symbols) {
  if (result === null) {
    ...
    // B30: distinguish "this specific name is missing" from "this file has no
    // function-like symbols at all" (unsupported file type, or a parse failure
    // that also defeated the regex fallback) — same generic wording previously
    // covered both cases, misleading anyone debugging what looked like a typo.
    if (symbols && symbols.length === 0) {
      msg += `\n⚠️  No function/symbol-like patterns were found anywhere in this file at all — this may not be a supported code file type, or tree-sitter parsing failed. Consider afterString/beforeString or afterLine/beforeLine instead.`;
    } else if (symbols && symbols.length > 0 && /^\w+$/.test(hintValue)) {
  ...
  if (result.ambiguous) {
    if (Array.isArray(result.matches)) {
      // resolveSymbolPosition shape — { ambiguous: true, matches: [{name,startRow,endRow,kind}...] }
      const list = result.matches
        .map(m => `  L${m.startRow + 1}–L${m.endRow + 1}: ${m.name}`)
        .join('\n');
      // The "use inFunction instead" tail is only useful advice for the anchor-style hints
      // (afterFunction/beforeFunction/afterSymbol/beforeSymbol), where the caller is placing a
      // position relative to a symbol. When the failing hint IS inFunction the caller has
      // already done exactly that, so suggesting it is circular -- omit it.
      const _scopeTail = hintName === 'inFunction' ? '' : ' To scope the edit inside a specific function body, use inFunction instead.';
      return `❌ ${hintName} "${hintValue}" is ambiguous — ${result.matches.length} functions with that name:\n${list}\nUse occurrence:N to disambiguate.${_scopeTail}`;
    }
    // resolveStringPosition shape — { ambiguous: true, rows: [number...] }
    const list = result.rows.map(r => `  L${r + 1}`).join('\n');
    return `❌ ${hintName} "${hintValue}" is ambiguous — appears on ${result.rows.length} lines:\n${list}\nUse a more unique anchor string, or occurrence:N to target a specific match.`;
  }
...
function smartSuggestion(ctx) {
  ...
  if (noHintsUsed) {
    if (toolName === "str_replace" || toolName === "replace-block") {
      ...
      parts.push("   inFunction:\"name\"     → scope search to a named function body (immune to line drift)");
      parts.push("   afterString:\"text\"    → start search after the line containing this text");
      parts.push("   beforeString:\"text\"   → start search before the line containing this text");
      ...
      // Consolidated tool (2026-09-17): replaces the retired delete-line / delete-line-range /
      // delete-block branches, which no longer match any registered tool name.
      parts.push("💡 NO HINTS USED — line numbers shift after every edit. Prefer content-stable alternatives:");
      parts.push("   inFunction:\"name\"          → delete an entire named function");
      parts.push("   startContent + endContent  → delete lines between two unique anchor strings");
      parts.push("   matchString:\"text\"         → delete one exact block by content");
  ...
  if (n >= 2) {
    const alts = {
      ...
      "delete":            "matchString (exact block by content), startContent+endContent (anchor pair), or sectionHint/preprocBlock — call get-structural-anchors to list available names; or str_replace with new_str:\"\" to remove specific content",
...
// ctx = { toolName, noHintsUsed, afterLineOnly, matchedLineContent,
//         fileLines, oldStr, isCodeFile, symbols }
// symbols (optional) — array from getSymbols() for the edited file. When
// provided and empty (B31), the file has no parseable function/symbol table
// (unsupported type, or a parse failure) — see the empty-symbols branch below.
// ---------------------------------------------------------------------------
function successNudge(ctx) {
  const { toolName, noHintsUsed, afterLineOnly, positionalHintName, matchedLineContent, fileLines, oldStr, isCodeFile, symbols } = ctx;
  // Case 1: a purely positional hint (afterLine or insert_line) was used with no
  // content-stable hint — suggest afterString upgrade. Include the matched line
  // content so the LLM can self-correct on the very next call.
  // B21: generalized from afterLine-only to also cover insert_line, which has
  // the identical silent-drift risk but previously got no targeted warning at
  // all — it fell through to the generic, higher-threshold Case 2 message.
  const _posName = positionalHintName || (afterLineOnly ? 'afterLine' : null);
  if (_posName && matchedLineContent && fileLines >= 100) {
    const anchor = matchedLineContent.trim().substring(0, 80).replace(/"/g, '\\"');
    return `\n⚡ ${_posName} is positional — it drifts if lines are inserted or deleted above it.` +
      `\n   Switch to afterString:"${anchor}"` +
      `\n   That anchor is content-stable and will find the right location even after edits.`;
  }
  ...
  // B31: this file has no parseable function/symbol table at all (unsupported
  // file type, e.g. .txt, or a parse failure) — inFunction/afterFunction/
  // afterSymbol can never resolve here, so recommending them next is dead
  // advice. Steer to a content anchor instead, using the line that was just
  // matched as a concrete drift-immune example.
  if (symbols && symbols.length === 0) {
    parts.push(`   This file has no parseable function/symbol table (unsupported file type or parse failure) — inFunction/afterFunction/afterSymbol won't work here.`);
    if (matchedLineContent) {
      const anchor = matchedLineContent.trim().substring(0, 80).replace(/"/g, '\\"');
      parts.push(`   Next time use afterString:"${anchor}" or beforeString instead — content anchors work on any file type.`);
    } else {
      parts.push(`   Next time use afterString/beforeString with a unique nearby line instead.`);
    }
    return parts.join("\n");
  }
  if (isCodeFile && oldStr) {
  ...
  parts.push(`   Next time add inFunction, afterString, or betweenHint to make the match drift-immune.`);
  return parts.join("\n");
}

// ---------------------------------------------------------------------------
// ambiguityCheck — shared by str_replace, replace-block, replace-function-body,
// delete, insert. Counts occurrences of needle in fullText and returns
...
//                 narrower text so inFunction/betweenHint scopes don't
...
function ambiguityCheck({ needle, fullText, scopedText, noScopeHint, toolName, isCodeFile, existingMatchLines }) {
  ...
  const hints = [
    ...
    `   • inFunction:"name"       → scope to a named function body`,
    `   • afterString:"string"      → start search after a unique anchor`,
    ...
    isCodeFile && (toolName === "delete" || toolName === "insert")
      ? `   • Or use delete/insert with sectionHint if this is a named section` : "",
  ].filter(Boolean).join("\n");
  return {
...
// ---------------------------------------------------------------------------
// scanForOldStr — called on str_replace noMatch when a scope hint was active.
// Scans the FULL buffer for old_str to detect "match exists but outside scope".
//
// needle     — the old_str that failed to match
// allLines   — full buffer lines array
// scopeStart — 0-based line index of the active scope start (or -1 if no scope)
// scopeEnd   — 0-based line index of the active scope end   (or -1 if no scope)
//
// Returns null when needle is not found anywhere in the full buffer.
// Returns { hitsOutsideScope, hitsInsideScope, total, truncated } when hits exist.
// Each hit: { line (1-based), funcCtx } where funcCtx is 'inside fn()' or 'top-level'.
// ---------------------------------------------------------------------------
function scanForOldStr({ needle, allLines, scopeStart, scopeEnd }) {
  if (!needle || !allLines || allLines.length === 0) return null;

  const fullText = allLines.join('\n');
  const hasScope = scopeStart >= 0 && scopeEnd >= 0 && scopeStart < scopeEnd;
  const MAX_HITS = 20;
  const hits     = [];
  let   pos      = 0;
  ...
  while (hits.length < MAX_HITS) {
    ...
    if (idx === -1) break;
    const lineNum = fullText.substring(0, idx).split('\n').length; // 1-based
    hits.push({ lineNum });
    pos = idx + 1;
  }
  ...
  // Lightweight function-context scanner — walks up from hit looking for a signature.
  const FN_SIG_RE = /^\s*(?:(?:async|static|export|function|def)\s+)?(\w+)\s*[=(]/;
  function getFuncCtx(lineNum) {
    const row = lineNum - 1; // 0-based
    for (let i = row; i >= Math.max(0, row - 40); i--) {
      const m = FN_SIG_RE.exec(allLines[i]);
      if (m) return `inside ${m[1]}()`;
    }
    return 'top-level';
  }

  const hitsInsideScope  = [];
  const hitsOutsideScope = [];
  for (const h of hits) {
    const row0   = h.lineNum - 1;
    const inside = hasScope && row0 >= scopeStart && row0 <= scopeEnd;
    const entry  = { line: h.lineNum, funcCtx: getFuncCtx(h.lineNum) };
    if (inside) hitsInsideScope.push(entry);
    else         hitsOutsideScope.push(entry);
  }
