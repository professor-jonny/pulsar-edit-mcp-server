...
function successNudge(ctx) {
  const { toolName, noHintsUsed, afterLineOnly, matchedLineContent, fileLines, oldStr, isCodeFile } = ctx;
  // Case 1: afterLine used but no content-stable hint — suggest afterString upgrade.
  // Include the matched line content so the LLM can self-correct on the very next call.
  if (afterLineOnly && matchedLineContent && fileLines >= 100) {
    const anchor = matchedLineContent.trim().substring(0, 80).replace(/"/g, '\\"');
    return `\n⚡ afterLine is positional — it drifts if lines are inserted or deleted above it.` +
