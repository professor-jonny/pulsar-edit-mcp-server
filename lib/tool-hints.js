  ...
  if (result.ambiguous) {
    if (Array.isArray(result.matches)) {
      ...
      const list = result.matches
        ...
        .join('\n');
      return `❌ ${hintName} "${hintValue}" is ambiguous — ${result.matches.length} functions with that name:\n${list}\nUse occurrence:N to disambiguate. To scope the edit inside a specific function body, use inFunction instead.`;
    }
...
function smartSuggestion(ctx) {
  ...
  if (noHintsUsed) {
    if (toolName === "str_replace" || toolName === "replace-block") {
      ...
      parts.push("   inFunction:\"name\"      → scope the anchor search to a function body");
