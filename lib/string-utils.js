...
// ---------------------------------------------------------------------------
// Helper: apply a replacement string with $1/$2/$&/$`/$'/$<name> interpolation.
...
//   rep         -- the replacement template string (may contain $1, $2, $&, $<name>, etc.)
...
//   namedGroups -- match.groups object (named capture groups for $<name> syntax); optional
// ---------------------------------------------------------------------------
function applyReplacement(rep, m, groups, pre, post, namedGroups) {
  return rep.replace(/\$(<([^>]+)>|\d+|&|`|')/g, function(_, token, namedKey) {
    if (namedKey !== undefined) {
      // $<name> named capture group
      return (namedGroups && namedGroups[namedKey] !== undefined) ? namedGroups[namedKey] : '';
    }
    if (token === '&') return m;
...
function globToRegex(glob) {
  ...
  // Use suffix match (no ^ anchor) so absolute paths match glob patterns like 'lib/*.js'
...
// ---------------------------------------------------------------------------
// normUnicode — normalise Unicode variants that commonly differ between an
// LLM-generated old_str and the actual buffer content, without touching real
// content differences: smart quotes -> straight, em/en dash -> hyphen,
// NBSP -> space, zero-width chars stripped, surrogate-pair emoji stripped.
// Extracted (B24) so match-engine.js and any tool can share one
// implementation instead of private copies drifting.
// ---------------------------------------------------------------------------
function normUnicode(str) {
  if (!str) return str;
  return str
    .replace(/[\u2018\u2019\u201A\u201B]/g, "'")
    .replace(/[\u201C\u201D\u201E\u201F]/g, '"')
...
module.exports = { escapeRegex, applyReplacement, expandBraces, globToRegex, levenshteinDistance, calculateSimilarity, normUnicode };
