...
const TOOL_CATALOGUE = [
  ...
  { name: "str_replace",             group: "edit",        desc: "Replace old_str with new_str. DECISION: know the function name? \u2192 inFunction. Know nearby unique text? \u2192 afterString or betweenHint. Have a line number? \u2192 afterLine:N (positional, 25-line window downward) or beforeLine:N (25-line window upward). Same pattern appears N times? \u2192 occurrence:N. String anchors work for unstructured files (markdown, config) too -- e.g. afterString:'## Installation' scopes an edit to under that heading, betweenHint:{start:'## Config',end:'## Usage'} restricts to that section. " +
    ...
    "Unsure of match? \u2192 dryRun:true first. new_str:\"\" deletes the matched old_str with nothing inserted -- for adding new lines without removing anything, use insert instead. RESPONSE: \u2705 str_replace \u2014 line N, \u00b1M lines [tags]. Tags: fuzzyWhitespace/fuzzyContent/regex when used. Followed by \u26a0\ufe0f style/lint warnings \u2014 act on them immediately." },
  { name: "insert",                  group: "edit",        desc: "Insert new lines into the active editor without touching existing content. Use afterFunction/beforeFunction (relative to a named function — drift-immune) or afterString/beforeString (relative to a unique line of text). dryRun previews position before writing — after a preview, call again with just { filePath, commitLastPreview: true } to apply it without resending new_str/anchors. String anchors work for unstructured files (markdown, config): afterString:'## Installation' inserts after that heading, beforeString:'## Usage' inserts before it. RESPONSE: ✅ insert — line N, +M lines [scopeLabel]. Followed by ⚠️ style/lint warnings if any issues in the inserted region." },
  ...
  { name: "delete",                  group: "edit",        desc: "Delete lines from a file, six ways in priority order: sectionHint (named banner block), preprocBlock (#ifdef..#endif by macro name; preprocSide:'open'|'close' for one end), inFunction (whole named function), startContent+endContent (content-anchored range; inclusive:false excludes the anchors), matchString (one exact block; supports fuzzyWhitespace/fuzzyContent/regex), or startLine+endLine (LEGACY/LAST RESORT -- add expectedContent to verify). occurrence:N disambiguates; an ambiguous anchor is refused with candidate lines, never guessed. Scope hints (afterFunction/beforeFunction, afterSymbol/beforeSymbol, afterString/beforeString, betweenHint, afterLine/beforeLine) narrow where matchString searches -- they do not define a range on their own. dryRun previews what will be removed. RESPONSE: ✅ delete — line N, -M lines. Followed by ⚠️ lint: if linter fires near the deletion point." },
  ...
  { name: "replace",                 group: "edit",        desc: "EXPERIMENTAL, engine-native sibling of str_replace, built for side-by-side comparison. old_str must match one or more WHOLE lines (no mid-line fragment matching, unlike str_replace) -- for a mid-line edit use str_replace instead. Same scope hints as str_replace/insert/delete (inFunction, afterString/beforeString, afterFunction/beforeFunction, afterSymbol/beforeSymbol, afterLine/beforeLine, betweenHint), plus occurrence, fuzzyWhitespace, fuzzyContent, regex, dryRun. Own stats namespace ('replace'), separate from str_replace's. RESPONSE: ✅ replace — line N, ±M lines. Followed by ⚠️ style/lint warnings if any." },
  { name: "read",                    group: "edit",        desc: "Read a section of any file at `filePath` without opening it, using any of the same anchor hints str_replace/insert/delete understand: inFunction, afterFunction/beforeFunction/afterSymbol/beforeSymbol, afterString/beforeString, an anchor pair (betweenHint or startContent+endContent), centerLine/nearLine+radius, or startLine+endLine. occurrence:N disambiguates; fuzzyWhitespace/fuzzyContent/regex supported. Immune to line drift on every anchor-based mode." },
  ...
  { name: "sed",                     group: "edit",        desc: "sed-style pattern-based editing on the file at `filePath` (required). expression is a s/pattern/replacement/flags string. dryRun:true previews matches. RESPONSE: ✅ sed — N match(es) [scope]. Change summary shown as nudge. Followed by ⚠️ lint: if linter fires after the edit." },
  { name: "get-structural-anchors",  group: "edit",        desc: "List section banners (sectionHint) and #ifdef blocks (preprocBlock) in the active file. Call this before insert or delete when you need an anchor name to use." },
  // FileOps
  { name: "get-project-files",       group: "fileOps",     desc: "List all files under project roots, optionally filtered by glob. Use to discover file paths before read or grep-file." },
  { name: "read-file",               group: "fileOps",     desc: "Read an entire file with 1-based line numbers. Auto-truncates on large files (part N of M, pass offset for the next chunk) to stay under the tool result size limit — prefer read (sections) or grep-file (search) when you don't need the whole file." },
  ...
  { name: "move-file",               group: "fileOps",     desc: "Move (or rename) a file from sourcePath to destPath." },
  { name: "copy-file",               group: "fileOps",     desc: "Copy a file to a new path." },
  ...
  { name: "rename-folder",           group: "fileOps",     desc: "Rename or move a folder. All files inside are moved with it." },
  { name: "get-includes-and-defines",group: "fileOps",     desc: "Return all #include and #define lines with line numbers from a C/C++ file." },
  { name: "list-project-functions",  group: "fileOps",     desc: "List every function definition with line numbers across all project files. Use to find where a function is defined before read or replace-function-body." },
  { name: "file-line-count",         group: "fileOps",     desc: "Return a file's line count without loading content. Use before read-file to decide whether to use read instead." },
  { name: "apply-patch",             group: "edit",     desc: "Apply a unified diff patch to the file at `filePath` (required). Context-anchored so it survives line shifts. Use for large multi-location edits where str_replace would require many calls. Previews by default (dryRun defaults to true) — pass dryRun:false to commit. RESPONSE: ✅ apply-patch — linesAdded/linesRemoved, N hunks [rescued if fuzzy rescue used]. Followed by ⚠️ style/lint warnings if any." },
  // Debugging
  ...
  // Diagnostics
  { name: "get-compiler-diagnostics", group: "diagnostics", desc: "Syntax-check the active C/C++ file with gcc/clang/cl (runs on the file as saved on disk; edit tools auto-save, so this is normally already current)." },
  ...
  { name: "insert-function-doc", group: "edit",    desc: "Insert a kernel-doc /** */ skeleton above a named function in the file at `filePath` (required). function_name() - desc, @param:, Context:, Return: sections. Parses parameter names from the signature; handles variadic @... args. Accepts optional line: (1-based, from check-function-docs) to disambiguate when functionName isn't unique. Kernel .c/.h files only." },
];
