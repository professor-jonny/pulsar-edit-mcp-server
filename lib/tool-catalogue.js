...
const TOOL_CATALOGUE = [
  ...
  { name: "get-failure-log",         group: "debugging",   desc: "Query the persistent failure-log.ndjson. Returns structured JSON entries. Filter by tool (e.g. 'str_replace'), reason (e.g. 'noMatch'), or filePath substring. tail:N limits output (default 20). Faster than PowerShell for diagnosing recurring failure patterns." },
  ...
  { name: "find-text",               group: "search",      desc: "Find all occurrences of a string or regex in the active editor. Returns line numbers and totalMatches count. Use occurrence:N to get the exact line of a specific instance (pass to str_replace lineNumberHint). Use contextLines:N to see surrounding code. caseSensitive defaults to false." },
  // Diagnostics
