...
//   FN_DEF_RE                      → exported regex for function definition matching
//
 ...
 * Captures [1]=optional 'static', [2]=function name.
 * Conservative — only fires on lines that look like definitions.
 */
const FN_DEF_RE = /^(?:[\w\s\*]+?\s+)?(static\s+)?(?:(?:[\w\*]+)\s+)+(\w+)\s*\(/;
...
function checkFunctionDocs(text, filePath, headerText) {
	...
	if (headerText) {
		...
		for (const hl of hlines) {
			const hm = hl.match(FN_DEF_RE);
			if (hm && hm[2] && hm[2].length > 2) headerNames.add(hm[2]);
		}
	}
...
module.exports = { checkNaming, checkFunctionDocs, buildDocSkeleton, formatNamingViolations, isKernelFile, FN_DEF_RE };
